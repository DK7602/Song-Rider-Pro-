...
function startNotesImageDrag(block, startEvent){
  ...
  try{ block.setPointerCapture?.(startEvent.pointerId); }catch{}
  state.notesImageDrag.holdTimer = setTimeout(() => {
    const drag = state.notesImageDrag;
    if(!drag || drag.id !== id || drag.block !== block) return;
    drag.active = true;
    drag.activationDx = Number(drag.lastX || 0) - Number(drag.startX || 0);
    drag.activationDy = Number(drag.lastY || 0) - Number(drag.startY || 0);
    block.classList.add('is-selected');
    block.style.opacity = '0.86';
...
function moveNotesImageDrag(ev){
  ...
  if(!drag.active){
    if(Math.abs(dxFromStart) > 16 || Math.abs(dyFromStart) > 16){
  ...
  try{
    ...
    target = hit?.closest?.('.notesImageBlock') || null;
  }catch{}
  if(!target || target === drag.block) return;
  ...
  if(before){
    if(target !== block.previousElementSibling){
      parent.insertBefore(block, target);
      if(spacerAfter) parent.insertBefore(spacerAfter, block.nextSibling);
      drag.didReorder = true;
    }
  }else{
    const targetAfter = spacerAfter && target === spacerAfter ? nextElem(target) : nextElem(target);
    const insertBefore = targetAfter ? nextElem(targetAfter) : null;
    if(insertBefore) parent.insertBefore(block, insertBefore);
