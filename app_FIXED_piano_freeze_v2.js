...
function parseChordToken(raw){
  ...
  qual = qual
    ...
    .replace(/^min/i,"m")
    .replace(/^minor/i,"m")
    .replace(/^n/i,"m")      // ✅ allow Nashville-style "n" = minor (e.g., F#n)
    .replace(/^-/,"m")
    .replace(/^M7/,"maj7")
    .replace(/Δ7/i,"maj7");
...
function ensurePianoFx(ctx){
  ...
  // ✅ NO-FEEDBACK ambience for piano (prevents mobile freeze)
  // Two short taps + gentle lowpass. No delay feedback loops.
  const dryBus = ctx.createGain();
  dryBus.gain.value = 0.95;

  const wetIn = ctx.createGain();
  wetIn.gain.value = 1.0;

  const d1 = ctx.createDelay(0.25);
  d1.delayTime.value = 0.070;

  const g1 = ctx.createGain();
  g1.gain.value = 0.14;

  const d2 = ctx.createDelay(0.25);
  d2.delayTime.value = 0.115;
  ...
  // wet taps
  dryBus.connect(wetIn);
  wetIn.connect(d1); d1.connect(g1);
  wetIn.connect(d2); d2.connect(g2);
  ...
  state._pianoFx = { ctx, dryBus, wet, nodes:[dryBus, wetIn, d1, g1, d2, g2, lp, wet] };
