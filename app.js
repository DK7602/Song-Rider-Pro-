...
saveBtn: $("saveBtn"),
  ...
  notesCropModal: $("notesCropModal"),
  notesCropViewport: $("notesCropViewport"),
  notesCropStage: $("notesCropStage"),
  notesCropImage: $("notesCropImage"),
  notesCropSelection: $("notesCropSelection"),
  notesCropCancelBtn: $("notesCropCancelBtn"),
  notesCropApplyBtn: $("notesCropApplyBtn"),
...
function serializeNotesEditorHtml(){
  ...
  clone.querySelectorAll('.notesImageResizeHandle,.notesImageDeleteBtn,.notesImageMoveUpBtn,.notesImageMoveDownBtn,.notesImageActionBar').forEach((node) => node.remove());
...
function startNotesImageDrag(block, startEvent){
  ...
  state.notesImageDrag = {
    ...
    holdTimer: setTimeout(() => {
      if(!state.notesImageDrag || state.notesImageDrag.id !== id) return;
      state.notesImageDrag.active = true;
      block.classList.add('is-selected');
      block.style.opacity = '0.82';
      block.style.transform = 'scale(0.985)';
    }, 360)
  };
}
...
function moveNotesImageDrag(ev){
  ...
  const dy = y - Number(drag.lastY || 0);
  if(Math.abs(dy) < 34) return;
  drag.lastY = y;
  drag.moved = true;
  moveNotesImageBlockBy(drag.id, dy < 0 ? -1 : 1);
}
...
function ensureNotesImageBlockChrome(block){
  ...
  let bar = block.querySelector('.notesImageActionBar');
  if(!bar){
    bar = document.createElement('div');
    bar.className = 'notesImageActionBar';
    bar.setAttribute('contenteditable','false');
    block.appendChild(bar);
  }
  const ensureActionBtn = (cls, label, title) => {
    let btn = bar.querySelector('.' + cls);
    if(btn) return btn;
    btn = document.createElement('button');
    btn.type = 'button';
    btn.className = 'notesImageActionBtn ' + cls;
    btn.setAttribute('contenteditable','false');
    btn.setAttribute('tabindex','-1');
    btn.setAttribute('title', title);
    btn.textContent = label;
  ...
  ensureActionBtn('notesImageCropBtn', 'Crop', 'Crop photo');
  ensureActionBtn('notesImageResetBtn', 'Reset', 'Reset photo position');
  ...
  syncNotesImageActionBarPosition(block);
}
...
async function resolveNotesImageElementSources(){
  ...
  for(const img of imgs){
    ...
    try{
      ...
      console.warn('Notes image remote download failed', err);
    }

    if(loaded) continue;

    const publicUrl = await getSignedOrPublicNoteImageUrl(entry.path);
    if(publicUrl){
      img.onload = () => { block?.classList.remove('is-loading'); };
      img.onerror = () => { block?.classList.remove('is-loading'); };
      img.src = publicUrl;
...
async function insertNotesImageFromFile(file){
  ...
  restoreNotesSelection();
  const html = `<div class="notesImageBlock" contenteditable="false" data-notes-image-id="${imageId}" style="width:min(100%, 420px);"><img class="notesImage" data-notes-image-id="${imageId}" alt="${escapeHtml(file.name || 'Photo')}" loading="lazy" decoding="async"></div><div><br></div>`;
  try{
...
async function getNotesImageRecord(imageId){
  const id = String(imageId || '').trim();
  if(!id) return null;
  try{ return await dbGetNoteImage(id); }catch{ return null; }
}

async function persistUpdatedNotesImageRecord(rec, reason='notes-image-update'){
  if(!rec || !rec.id || !state.project) return false;
  rec.updatedAt = now();
  rec.size = Number(rec.blob?.size || rec.size || 0);
  rec.contentType = String(rec.blob?.type || rec.contentType || 'image/jpeg');
  await dbPutNoteImage(rec);

  editProject(reason, () => {
    const manifest = getProjectNotesImages(state.project).filter((item) => String(item.id || '') !== String(rec.id || ''));
    manifest.unshift(noteImageToManifestEntry(rec));
    setProjectNotesImages(state.project, manifest);
  });
  ...
  if(cloud.ready && cloud.client && getAuthUserId()){
    try{
      ...
      console.warn('Notes image cloud update failed', err);
    }
  }
  ...
  saveNotesDoc();
  pushNotesHistorySnapshot(true);
  if(cloud.ready && cloud.client && getAuthUserId()) triggerImmediateCloudSync(reason);
  return true;
}
  ...
  return new Promise((resolve, reject) => {
    try{
      canvas.toBlob((blob) => {
        ...
        else reject(new Error('Could not create cropped image blob'));
      }, type, quality);
    }catch(err){
      reject(err);
    }
...
function getSelectedNotesImageBlock(){
  const id = String(state.notesSelectedImageId || '').trim();
  if(!id || !el?.notesEditor) return null;
  return el.notesEditor.querySelector(`.notesImageBlock[data-notes-image-id="${id}"]`);
}

function resetNotesImageTransform(block){
  if(!block) return;
  block.dataset.freeX = '0';
  block.dataset.freeY = '0';
  block.style.transform = 'translate(0px, 0px)';
}

function syncNotesImageActionBarPosition(block){
  if(!block) return;
  const bar = block.querySelector('.notesImageActionBar');
  if(!bar) return;
  bar.style.opacity = '';
}

function ensureNotesCropModal(){
  if(!el?.notesCropModal || !el?.notesCropViewport || !el?.notesCropImage || !el?.notesCropSelection) return;
  if(el.notesCropModal.dataset.wired === '1') return;
  el.notesCropModal.dataset.wired = '1';

  const getSelRect = () => {
    const stageRect = el.notesCropStage?.getBoundingClientRect?.();
    const selRect = el.notesCropSelection?.getBoundingClientRect?.();
    if(!stageRect || !selRect) return null;
    return {
      left: selRect.left - stageRect.left,
  ...
  const applyRect = (rect) => {
    if(!rect || !el?.notesCropSelection || !el?.notesCropStage) return;
    const stageRect = el.notesCropStage.getBoundingClientRect();
    const maxW = Math.max(40, stageRect.width || rect.stageW || 0);
    const maxH = Math.max(40, stageRect.height || rect.stageH || 0);
    const left = clamp(Math.round(rect.left || 0), 0, Math.max(0, maxW - 40));
    const top = clamp(Math.round(rect.top || 0), 0, Math.max(0, maxH - 40));
    const width = clamp(Math.round(rect.width || 40), 40, maxW - left);
    const height = clamp(Math.round(rect.height || 40), 40, maxH - top);
    el.notesCropSelection.style.left = left + 'px';
    el.notesCropSelection.style.top = top + 'px';
    el.notesCropSelection.style.width = width + 'px';
    el.notesCropSelection.style.height = height + 'px';
  };

  const stopCropDrag = () => {
    const crop = state.notesImageCrop;
    if(crop?.drag?.move) document.removeEventListener('pointermove', crop.drag.move, true);
    if(crop?.drag?.up){
      document.removeEventListener('pointerup', crop.drag.up, true);
      document.removeEventListener('pointercancel', crop.drag.up, true);
    }
    if(crop) crop.drag = null;
  };

  const startCropDrag = (ev, mode, handleDir='') => {
    if(!state.notesImageCrop || !el?.notesCropStage || !el?.notesCropSelection) return;
    const start = getSelRect();
    if(!start) return;
    ev.preventDefault();
    ev.stopPropagation();
    state.notesImageCrop.drag = {
      mode,
      handleDir,
      startPointerX: Number(ev.clientX || 0),
      startPointerY: Number(ev.clientY || 0),
      startRect: start,
      move: null,
      up: null
    };
    const move = (e2) => {
      ...
      if(!crop?.drag) return;
      const stageRect = el.notesCropStage.getBoundingClientRect();
      const dx = Number(e2.clientX || 0) - crop.drag.startPointerX;
      const dy = Number(e2.clientY || 0) - crop.drag.startPointerY;
      const rect = {
        left: crop.drag.startRect.left,
        top: crop.drag.startRect.top,
        width: crop.drag.startRect.width,
        height: crop.drag.startRect.height
      };
      if(crop.drag.mode == 'move'){
        rect.left += dx;
        rect.top += dy;
      }else{
        const dir = String(crop.drag.handleDir || '');
        if(dir.includes('w')) { rect.left += dx; rect.width -= dx; }
        if(dir.includes('e')) { rect.width += dx; }
        if(dir.includes('n')) { rect.top += dy; rect.height -= dy; }
        if(dir.includes('s')) { rect.height += dy; }
      }
      rect.width = Math.max(40, rect.width);
      rect.height = Math.max(40, rect.height);
      rect.left = clamp(rect.left, 0, Math.max(0, stageRect.width - rect.width));
      rect.top = clamp(rect.top, 0, Math.max(0, stageRect.height - rect.height));
      applyRect(rect);
      try{ e2.preventDefault(); e2.stopPropagation(); }catch{}
    };
    const up = () => stopCropDrag();
    state.notesImageCrop.drag.move = move;
    state.notesImageCrop.drag.up = up;
    document.addEventListener('pointermove', move, { passive:false, capture:true });
    document.addEventListener('pointerup', up, true);
    document.addEventListener('pointercancel', up, true);
  };

  el.notesCropSelection?.addEventListener('pointerdown', (ev) => {
    const handle = ev.target?.closest?.('.notesCropHandle');
    if(handle){
      startCropDrag(ev, 'resize', String(handle.dataset.dir || ''));
      return;
    }
    startCropDrag(ev, 'move', '');
  });

  el.notesCropCancelBtn?.addEventListener('click', () => closeNotesCropModal());
  document.getElementById('notesCropCancelBtn2')?.addEventListener('click', () => closeNotesCropModal());
  el.notesCropModal?.addEventListener('click', (ev) => {
    if(ev.target === el.notesCropModal) closeNotesCropModal();
  });
  el.notesCropApplyBtn?.addEventListener('click', async () => {
    const crop = state.notesImageCrop;
    if(!crop?.imageId || !crop?.sourceBlob || !el?.notesCropImage || !el?.notesCropSelection || !el?.notesCropStage) return;
    try{
      el.notesCropApplyBtn.disabled = true;
      const srcImg = await loadImageFromBlob(crop.sourceBlob);
      const stageRect = el.notesCropStage.getBoundingClientRect();
      const imgRect = el.notesCropImage.getBoundingClientRect();
      const selRect = el.notesCropSelection.getBoundingClientRect();

      const relLeft = clamp(selRect.left - imgRect.left, 0, imgRect.width);
      const relTop = clamp(selRect.top - imgRect.top, 0, imgRect.height);
      const relWidth = clamp(selRect.width, 1, imgRect.width - relLeft);
      const relHeight = clamp(selRect.height, 1, imgRect.height - relTop);

      const scaleX = srcImg.naturalWidth / Math.max(1, imgRect.width);
      const scaleY = srcImg.naturalHeight / Math.max(1, imgRect.height);

      const sx = Math.round(relLeft * scaleX);
      const sy = Math.round(relTop * scaleY);
      const sw = Math.max(1, Math.round(relWidth * scaleX));
      const sh = Math.max(1, Math.round(relHeight * scaleY));
      ...
      const outType = /^image\/png$/i.test(String(crop.sourceBlob.type || '')) ? 'image/png' : 'image/jpeg';
      const blob = await canvasToBlob(canvas, outType, 0.92);
      const rec = await getNotesImageRecord(crop.imageId);
      if(rec){
        rec.blob = blob;
        rec.contentType = blob.type || outType;
        rec.size = Number(blob.size) || 0;
        rec.title = String(rec.title || crop.title || 'Photo');
        await persistUpdatedNotesImageRecord(rec, 'notes-image-crop');
      }
      closeNotesCropModal();
      selectNotesImageBlockById(crop.imageId);
    }catch(err){
      console.error(err);
      alert('Could not crop photo.');
    }finally{
      if(el?.notesCropApplyBtn) el.notesCropApplyBtn.disabled = false;
    }
  });
}

function openNotesCropModal(imageId){
  const id = String(imageId || state.notesSelectedImageId || '').trim();
  if(!id || !el?.notesCropModal || !el?.notesCropImage || !el?.notesCropSelection || !el?.notesCropStage) return;
  ensureNotesCropModal();
  (async () => {
    const rec = await getNotesImageRecord(id);
    if(!rec?.blob) return;
    closeNotesCropModal(true);
    const url = URL.createObjectURL(rec.blob);
    state.notesImageCrop = {
      imageId: id,
      sourceBlob: rec.blob,
      title: String(rec.title || 'Photo'),
      objectUrl: url,
      drag: null
    };
    el.notesCropImage.onload = () => {
      const stageRect = el.notesCropStage.getBoundingClientRect();
      const imgRect = el.notesCropImage.getBoundingClientRect();
      const left = Math.max(0, imgRect.left - stageRect.left);
      const top = Math.max(0, imgRect.top - stageRect.top);
      const width = Math.max(80, imgRect.width);
      const height = Math.max(80, imgRect.height);
      const insetW = Math.round(width * 0.1);
      const insetH = Math.round(height * 0.1);
      el.notesCropSelection.style.left = (left + insetW) + 'px';
      el.notesCropSelection.style.top = (top + insetH) + 'px';
      el.notesCropSelection.style.width = Math.max(60, width - insetW * 2) + 'px';
      el.notesCropSelection.style.height = Math.max(60, height - insetH * 2) + 'px';
    };
    el.notesCropImage.src = url;
    el.notesCropModal.classList.add('show');
  })().catch(console.error);
}

function closeNotesCropModal(skipRevoke=false){
  stopNotesImageResize(false);
  const crop = state.notesImageCrop;
  if(crop?.drag){
    try{ crop.drag.up?.(); }catch{}
  }
  if(!skipRevoke){
    try{ if(crop?.objectUrl) URL.revokeObjectURL(crop.objectUrl); }catch{}
  }
  if(el?.notesCropModal) el.notesCropModal.classList.remove('show');
  if(el?.notesCropImage){
    el.notesCropImage.onload = null;
    el.notesCropImage.src = '';
  }
  state.notesImageCrop = null;
}
  ...
  if(el.notesEditor){
    ...
    el.notesEditor.addEventListener("click", (e) => {
      ...
      if(imageCropBtn){
        ...
        if(block){
          selectNotesImageBlockById(block.getAttribute('data-notes-image-id'));
          openNotesCropModal(block.getAttribute('data-notes-image-id'));
        }
        return;
      }
      const imageResetBtn = e.target?.closest?.('.notesImageResetBtn');
      if(imageResetBtn){
        e.preventDefault();
        e.stopPropagation();
        const block = imageResetBtn.closest('.notesImageBlock');
        if(block){
          resetNotesImageTransform(block);
          saveNotesDoc();
          pushNotesHistorySnapshot(true);
          if(cloud.ready && cloud.client && getAuthUserId()) triggerImmediateCloudSync('notes-image-move');
        }
    ...
    el.notesEditor.addEventListener('pointerdown', (e) => {
      ...
      const moveBtn = e.target?.closest?.('.notesImageDeleteBtn,.notesImageActionBtn');
      if(moveBtn) return;
      const imageBlock = e.target?.closest?.('.notesImageBlock');
      if(imageBlock){
        selectNotesImageBlockById(imageBlock.getAttribute('data-notes-image-id'));
