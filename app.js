...
function parseFullTextToSectionCards(fullText){
  ...
  function pushCard(sec, lyricsRaw, slots, renderLines){
    ...
    buckets[sec].push({
      lyrics,
      chords,
      importRenderLines: Array.isArray(renderLines) ? renderLines.map(v => String(v ?? "")) : [],
      importLyrics: lyrics,
      importChords: chords.slice(0,8)
    });
  }
    ...
    for(let i=0; i<lines.length; i++){
      ...
      if(_br){
        ...
        if(pendingChordLine && lineIsMostlyChords(pendingChordLine)){
          pushCard(cur, "", buildSlotsFromAlignedChordLine(pendingChordLine, ""), [String(pendingChordLine || "")]);
        }
      ...
      const sourceChordLine = pendingChordLine ? String(pendingChordLine || "") : "";
      if(pendingChordLine){
      ...
      pushCard(cur, cleaned, slots, [sourceChordLine, String(line || "")].filter(Boolean));
    }
...
function applyFullTextToProjectSections(fullText){
  ...
  secsToApply.forEach(sec => {
    ...
    for(let i=0; i<want.length; i++){
      ...
      base.importLyrics = String(w.importLyrics || w.lyrics || "");
      base.importChords = Array.isArray(w.importChords) ? w.importChords.slice(0,8) : Array.isArray(w.chords) ? w.chords.slice(0,8) : Array(8).fill("");
      while(base.importChords.length < 8) base.importChords.push("");
      base.importRenderLines = Array.isArray(w.importRenderLines) ? w.importRenderLines.map(v => String(v ?? "")) : [];
...
function buildFullPreviewText(){
  const raw = String(state?.project?.fullText || "");
  const text = normalizeLineBreaks(raw).replace(/\s+$/g, "");
  return text.trim() ? text : "(No lyrics/notes yet - start typing in a section)";
}
...
function buildFullPreviewHtmlDoc(title){
  ...
  const fullText = buildFullPreviewText();
  ...
  const isHeadingLineForExport = (line) => {
    const raw = String(line || "").trim();
    if(!raw) return false;
    const br = _parseBreakLine(raw);
    if(br) return true;
    return !!headingTitleFromLine(raw) || !!isSectionHeadingLine(raw);
  };
  ...
  for(const rawLine of lines){
    ...
    if(isHeadingLineForExport(trim) && !lineLooksMostlyLikeChordsGlobal(trim)){
      blocks.push(`<div style="font-family:Arial,Helvetica,sans-serif;font-weight:900;margin:20px 0 8px;letter-spacing:.3px;">${escapeHtml(trim)}</div>`);
      continue;
    }

    const isChordLine = lineLooksMostlyLikeChordsGlobal(line);
    blocks.push(
      `<pre style="margin:0 0 ${isChordLine ? '0' : '10px'} 0;font-family:'Courier New',Courier,monospace;font-weight:${isChordLine ? '900' : '400'};color:${isChordLine ? '#7f1d1d' : '#111'};white-space:pre;">${escapeHtml(line)}</pre>`
    );
  }
...
function chordArraysMatch8(a, b){
  const aa = Array.from({length:8}, (_,i)=> String((Array.isArray(a) ? a[i] : "") || "").trim());
  const bb = Array.from({length:8}, (_,i)=> String((Array.isArray(b) ? b[i] : "") || "").trim());
  for(let i=0;i<8;i++) if(aa[i] !== bb[i]) return false;
  return true;
}

function canReuseImportedRender(line){
  if(!line || typeof line !== "object") return false;
  const renderLines = Array.isArray(line.importRenderLines) ? line.importRenderLines.map(v => String(v ?? "")) : [];
  if(!renderLines.length) return false;
  const importLyrics = String(line.importLyrics || "").trim();
  const currentLyrics = String(line.lyrics || "").trim();
  if(importLyrics !== currentLyrics) return false;
  return chordArraysMatch8(line.importChords, line.notes);
}
  ...
  try{
    ...
    getVisibleSectionPages().filter(s => s !== "Full").forEach(sec => {
      ...
      for(const line of arr){
        ...
        wroteAny = true;
        if(canReuseImportedRender(line)){
          const importedLines = line.importRenderLines.map(v => String(v ?? ""));
          importedLines.forEach(srcLine => out.push(srcLine));
          out.push(""); // blank line = next card
          continue;
        }
