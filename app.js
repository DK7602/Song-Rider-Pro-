...
function seekActiveAudioTo(sec, opts={}){
  ...
  try{
    // fastSeek is useful where supported; currentTime remains the reliable fallback.
    if(opts.fast && typeof a.fastSeek === "function") a.fastSeek(target);
  ...
  // During a finger drag, immediately show the requested time but DO NOT let
  // the normal playback refresh overwrite the range thumb.
...
function wireAudioProgressControl(){
  ...
  const paintScrubPosition = (target) => {
    const a = state.audioSyncAudio;
  ...
  const scrubToX = (clientX, finalCommit=false) => {
    if(!canSeek()) return;
    const target = timeFromClientX(clientX);
    paintScrubPosition(target);

    // Set the real HTMLAudioElement position directly.
    seekActiveAudioTo(target, { scrubbing:true });

    if(finalCommit){
      state.audioSeekDragging = false;
      state.audioSeekPreviewSec = null;
      requestAnimationFrame(() => refreshAudioPlaybackUI());
      setTimeout(() => refreshAudioPlaybackUI(), 100);
    }
  };
  ...
  const onPointerDown = (e) => {
    ...
    scrubToX(e.clientX, false); // tap anywhere jumps there immediately
  };

  const onPointerMove = (e) => {
    if(!state.audioSeekDragging) return;
  ...
  bar.addEventListener("pointercancel", onPointerUp, { passive:false });
  ...
  bar.addEventListener("keydown", (e) => {
    ...
    e.preventDefault();
    seekActiveAudioTo(clamp(target, 0, a.duration));
  });
}
