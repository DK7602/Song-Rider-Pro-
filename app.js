...
function moveNotesImageDrag(ev){
  ...
  drag.startY = Number(ev?.clientY || 0);
  drag.moved = true;
...
function ensureNotesImageBlockChrome(block){
  ...
  makeBtn('notesImageMoveUpBtn', '↑', 'Move photo up');
  makeBtn('notesImageMoveDownBtn', '↓', 'Move photo down');
...
if(el.capoInput){
  ...
  if(el.notesEditor){
    ...
    el.notesEditor.addEventListener('pointerdown', (e) => {
      ...
      const moveBtn = e.target?.closest?.('.notesImageMoveUpBtn, .notesImageMoveDownBtn, .notesImageDeleteBtn');
      if(moveBtn) return;
      const imageBlock = e.target?.closest?.('.notesImageBlock');
      if(imageBlock){
        selectNotesImageBlockById(imageBlock.getAttribute('data-notes-image-id'));
