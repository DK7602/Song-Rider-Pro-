...
horseLeft: $("horseLeft"),
  ...
  audioProgressWrap: $("audioProgressWrap"),
  audioProgress: $("audioProgress"),
  audioProgressFill: $("audioProgressFill"),
  audioProgressThumb: $("audioProgressThumb"),
...
function parseNotesBackupRaw(raw){
  try{
    ...
    const notesClearedAt = Number(parsed.notesClearedAt) || 0;
    return {
...
function mergeNotesSafely(primary, ...fallbacks){
  ...
  // pickBestNotesBackup ignores unmarked accidental blanks, but treats an
  // explicit user-cleared blank as a real timestamped state.
  return pickBestNotesBackup(candidates) || primary || null;
}
...
function pickBestNotesBackup(items){
  ...
  // A blank note is only authoritative when it was explicitly cleared by the user.
  // Ordinary blank payloads are still treated as possible load/render failures.
  const validStates = list.filter((item) => {
    const doc = String(item?.notesDoc || "");
    return notesDocHasMeaningfulContent(doc) || (Number(item?.notesClearedAt) || 0) > 0;
  });
  const pool = validStates.length ? validStates : list;

  let best = null;
  let bestTime = -Infinity;
  for(const item of pool){
    const t = Math.max(
      Number(item?.notesUpdatedAt) || 0,
      Number(item?.updatedAt) || 0,
      Number(item?.notesClearedAt) || 0
    );
    if(t > bestTime){
      best = item;
      bestTime = t;
...
function writeNotesBackup(project, opts={}){
  try{
    if(!project || !project.id) return;

    const incomingDoc = String(project.notesDoc || "");
    const incomingHasNotes = notesDocHasMeaningfulContent(incomingDoc);
    const allowBlankOverwrite = !!opts.allowBlankOverwrite;
    const existing = readNotesBackup(project.id);
    const existingHasNotes = notesDocHasMeaningfulContent(existing?.notesDoc || "");

    // Keep protection against accidental blank saves, but allow a deliberate
    // user delete-all to become the authoritative state for this project.
    const protectedCopy = (!allowBlankOverwrite && !incomingHasNotes && existingHasNotes) ? existing : null;
    ...
    const payload = {
      ...
      notesUpdatedAt,
      notesClearedAt: protectedCopy
        ? (Number(protectedCopy.notesClearedAt) || 0)
        : (allowBlankOverwrite && !incomingHasNotes ? (Number(project.notesClearedAt) || notesUpdatedAt) : (Number(project.notesClearedAt) || 0)),
      updatedAt: notesUpdatedAt
    };
  ...
  for(const [id, row] of remoteMap.entries()){
    ...
    if(rowUpdated > localUpdated){
      const bestNotes = mergeNotesSafely(
        { notesDoc: remoteProject.notesDoc, notesColor: remoteProject.notesColor, notesUpdatedAt: Number(remoteProject.notesUpdatedAt) || Number(remoteProject.updatedAt) || rowUpdated, notesClearedAt: Number(remoteProject.notesClearedAt) || 0, updatedAt: Number(remoteProject.notesUpdatedAt) || Number(remoteProject.updatedAt) || rowUpdated },
        { notesDoc: local.notesDoc, notesColor: local.notesColor, notesUpdatedAt: Number(local.notesUpdatedAt) || 0, notesClearedAt: Number(local.notesClearedAt) || 0, updatedAt: Number(local.notesUpdatedAt) || 0 },
        readNotesBackup(id)
      );
      if(bestNotes){
        remoteProject.notesDoc = String(bestNotes.notesDoc || "");
        remoteProject.notesClearedAt = Number(bestNotes.notesClearedAt) || 0;
...
async function reloadProjectForCurrentScope(){
  ...
  if(prevProject && state.project && prevProject.id === state.project.id){
    const bestNotes = mergeNotesSafely(
      { notesDoc: state.project.notesDoc, notesColor: state.project.notesColor, notesUpdatedAt: Number(state.project.notesUpdatedAt) || 0, notesClearedAt: Number(state.project.notesClearedAt) || 0, updatedAt: Number(state.project.notesUpdatedAt) || 0 },
      { notesDoc: prevProject.notesDoc, notesColor: prevProject.notesColor, notesUpdatedAt: Number(prevProject.notesUpdatedAt) || 0, notesClearedAt: Number(prevProject.notesClearedAt) || 0, updatedAt: Number(prevProject.notesUpdatedAt) || 0 },
      readNotesBackup(state.project.id)
    );
    if(bestNotes){
      state.project.notesDoc = String(bestNotes.notesDoc || "");
      state.project.notesClearedAt = Number(bestNotes.notesClearedAt) || 0;
...
function hydrateProjectsArrayWithNotes(projects, scope){
  ...
  const out = arr.map((proj) => {
    ...
    candidates.push({
      ...
      notesClearedAt: Number(next.notesClearedAt) || 0,
    ...
    if(best){
      ...
      const bestNotesClearedAt = Number(best.notesClearedAt) || 0;
      ...
      if(bestNotesClearedAt !== (Number(next.notesClearedAt) || 0)){
        next.notesClearedAt = bestNotesClearedAt;
        changed = true;
      }
      ...
      if(!Number.isFinite(Number(next.notesClearedAt))){
        next.notesClearedAt = 0;
        changed = true;
      }
    }
...
function normalizeProject(p){
  ...
  p.notesClearedAt = Number(p.notesClearedAt) || 0;
    ...
    const bestNotes = mergeNotesSafely(
      {
        ...
        notesClearedAt: Number(p.notesClearedAt) || 0,
        updatedAt: Number(p.notesUpdatedAt) || 0
      },
      notesBackup
    );
    if(bestNotes){
      p.notesDoc = String(bestNotes.notesDoc || "");
      p.notesClearedAt = Number(bestNotes.notesClearedAt) || 0;
...
lastAutoBar: -1,
  ...
  audioSyncPaused: false,
  audioSeekDragging: false,
  audioSeekPreviewSec: null,
  lastAudioTick8: -1,
  tapTimes: [],
...
function updateAudioProgressUI(){
  ...
  const active = !!(a && state.audioSyncRecId);
  if(el.audioProgressWrap) el.audioProgressWrap.style.display = active ? "flex" : "none";

  if(!active){
    state.audioSeekDragging = false;
    state.audioSeekPreviewSec = null;
    if(el.audioProgress){
      el.audioProgress.dataset.enabled = "0";
      el.audioProgress.setAttribute("aria-disabled", "true");
      el.audioProgress.setAttribute("aria-valuenow", "0");
      el.audioProgress.setAttribute("aria-valuemax", "0");
    }
    if(el.audioProgressFill) el.audioProgressFill.style.width = "0%";
    if(el.audioProgressThumb) el.audioProgressThumb.style.left = "0%";
  ...
  const actualCurrent = Math.max(0, Math.min(Number(a.currentTime) || 0, duration || Infinity));
  const preview = Number(state.audioSeekPreviewSec);
  const shownCurrent = (state.audioSeekDragging && Number.isFinite(preview))
    ? Math.max(0, Math.min(preview, duration || Infinity))
    : actualCurrent;

  const pct = duration > 0 ? clamp((shownCurrent / duration) * 100, 0, 100) : 0;

  if(el.audioProgress){
    el.audioProgress.dataset.enabled = duration > 0 ? "1" : "0";
    el.audioProgress.setAttribute("aria-disabled", duration > 0 ? "false" : "true");
    el.audioProgress.setAttribute("aria-valuemin", "0");
    el.audioProgress.setAttribute("aria-valuemax", String(duration || 0));
    el.audioProgress.setAttribute("aria-valuenow", String(shownCurrent || 0));
  }
  if(el.audioProgressFill) el.audioProgressFill.style.width = pct + "%";
  if(el.audioProgressThumb) el.audioProgressThumb.style.left = pct + "%";
  if(el.audioCurrentTime) el.audioCurrentTime.textContent = formatAudioTime(shownCurrent);
  if(el.audioDuration) el.audioDuration.textContent = formatAudioTime(duration);
}

function updateRecordingPlayButtons(){
  try{
    document.querySelectorAll('[data-audio-play-rec-id]').forEach(btn => {
      const same = String(btn.dataset.audioPlayRecId || "") === String(state.audioSyncRecId || "");
      const paused = !!state.audioSyncPaused || !!state.audioSyncAudio?.paused;
      btn.textContent = (same && state.audioSyncOn && !paused) ? "⏸" : "▶";
      btn.title = (same && state.audioSyncOn && !paused) ? "Pause" : (same && state.audioSyncOn ? "Resume" : "Play");
      btn.setAttribute("aria-label", btn.title);
    });
  }catch{}
}

function refreshAudioPlaybackUI(){
  updateAudioProgressUI();
  updateRecordingPlayButtons();
}

async function toggleAudioSyncFromRec(rec){
  if(!rec || !rec.blob) return;
  const same = state.audioSyncOn && state.audioSyncAudio && String(state.audioSyncRecId || "") === String(rec.id || "");

  if(same){
    const a = state.audioSyncAudio;
    if(!a.paused && !state.audioSyncPaused){
      try{ a.pause(); }catch{}
      state.audioSyncPaused = true;
      if(state.audioSyncRaf){
        cancelAnimationFrame(state.audioSyncRaf);
        state.audioSyncRaf = null;
      }
      stopHorse();
      refreshAudioPlaybackUI();
      return;
    }
    ...
    try{
      await a.play();
      state.audioSyncPaused = false;
      state.lastAudioTick8 = -1;
      state.lastHorseBar = -1;
      if(state.audioSyncRaf) cancelAnimationFrame(state.audioSyncRaf);
      state.audioSyncRaf = requestAnimationFrame(audioSyncFrame);
      refreshAudioPlaybackUI();
    }catch(e){
      alert("Couldn't resume audio. Tap Play again to allow audio.");
    }
    return;
  }
...
function seekActiveAudioTo(sec, opts={}){
  const a = state.audioSyncAudio;
  if(!state.audioSyncOn || !a) return false;

  const duration = Number.isFinite(a.duration) && a.duration > 0 ? a.duration : 0;
  let target = Number(sec) || 0;
  if(duration > 0) target = clamp(target, 0, duration);
  else target = Math.max(0, target);

  state.audioSeekPreviewSec = target;
  ...
  state.lastAudioTick8 = -1;
  state.lastHorseBar = -1;
  const tick8 = audioTickFromTimeSec(target);
  state.tick8 = Math.max(0, tick8);
  clearTick();
  try{ if(shouldTickRun()) applyTick(); }catch{}

  if(el.audioCurrentTime) el.audioCurrentTime.textContent = formatAudioTime(target);
  if(!opts.scrubbing) refreshAudioPlaybackUI();
  return wrote;
}

function waitForAudioSeek(a, target, timeoutMs=700){
  return new Promise((resolve) => {
    ...
    const finish = (ok) => {
      ...
      try{ a.removeEventListener("seeked", onSeeked); }catch{}
      try{ a.removeEventListener("timeupdate", onTimeUpdate); }catch{}
      clearTimeout(timer);
      resolve(ok);
    };
    const closeEnough = () => Math.abs((Number(a.currentTime)||0) - target) < 0.35;
    const onSeeked = () => finish(closeEnough() || true);
    const onTimeUpdate = () => { if(closeEnough()) finish(true); };

    try{ a.addEventListener("seeked", onSeeked, { once:true }); }catch{}
    try{ a.addEventListener("timeupdate", onTimeUpdate); }catch{}

    const timer = setTimeout(() => finish(closeEnough()), timeoutMs);
    if(closeEnough()) finish(true);
  });
}

async function commitActiveAudioSeek(target, shouldResume){
  const a = state.audioSyncAudio;
  if(!state.audioSyncOn || !a) return;

  const duration = Number.isFinite(a.duration) && a.duration > 0 ? a.duration : 0;
  target = duration > 0 ? clamp(Number(target)||0, 0, duration) : Math.max(0, Number(target)||0);

  // Android is much more reliable when a scripted seek is committed while paused.
  ...
  seekActiveAudioTo(target, { scrubbing:true });
  await waitForAudioSeek(a, target);

  // Some Android media stacks report "seeked" before currentTime settles.
  // Re-assert the target once after the seek event as a safety net.
  try{
    if(Math.abs((Number(a.currentTime)||0) - target) > 0.35){
      a.currentTime = target;
      await waitForAudioSeek(a, target, 350);
    }
  }catch{}
  ...
  if(shouldResume && state.audioSyncOn && state.audioSyncAudio === a){
    try{
      const ctx = ensureCtx();
      if(ctx && ctx.state === "suspended") await ctx.resume();
    }catch{}
...
// Full Song View syllable counts.
// Reuses the exact card-view syllable counter + red/yellow/green thresholds.
function refreshFullSectionSyllableOverlay(ta, overlay){
  ...
  let cs;
  try{ cs = getComputedStyle(ta); }catch{ cs = null; }
  if(!cs) return;

  // Build an invisible textarea-width mirror so browser wrapping—not a
  // character-width estimate—decides where each logical line actually ends.
  // This keeps the syllable number directly after the last word even when a
  // long lyric wraps onto a second/third visual row on phones.
  ...
  mirror.style.width = `${ta.getBoundingClientRect().width || ta.clientWidth || 0}px`;
  ...
  lines.forEach((raw) => {
    ...
    // A logical newline must always start a new row, while the row itself can
    // wrap exactly like the textarea content.
    row.style.display = "block";
    row.style.minHeight = cs.lineHeight;
    row.style.whiteSpace = "pre-wrap";
    row.style.overflowWrap = "break-word";
    row.style.wordBreak = "break-word";

    const visibleText = String(raw || "").replace(/\s+$/g, "");
    if(visibleText) row.appendChild(document.createTextNode(visibleText));
  ...
  markers.forEach(({raw, marker}) => {
    const lyric = String(raw || "").trim();
    if(!lyric) return;
    if(_fullPerf_isHeadingLine(lyric)) return;
    if(_fullPerf_isChordLine(lyric)) return;

    const n = countSyllablesInline(lyric);
    if(!n) return;

    const mr = marker.getBoundingClientRect();
    const badge = document.createElement("span");
    badge.className = `fullLineSyllable ${syllToneClass(n)}`;
    badge.textContent = String(n);
    badge.setAttribute("aria-label", `${n} syllables`);

    // Marker is zero-width and therefore does not influence wrapping.
    // Put the number a small space after the actual last rendered word.
    badge.style.left = `${Math.round(mr.left - overlayRect.left + 6)}px`;
    badge.style.top = `${Math.round(mr.top - overlayRect.top)}px`;
...
function appendFullPerfSyllableCount(boxes, lyric){
  if(!Array.isArray(boxes) || !boxes.length) return;
  const n = countSyllablesInline(lyric || "");
  if(!n) return;
  let target = null;
  for(let i=boxes.length-1;i>=0;i--){
    if(String(boxes[i]?.textContent || "").trim()){ target = boxes[i]; break; }
  }
  target = target || boxes[boxes.length-1];
  if(!target) return;
  const badge = document.createElement("span");
  badge.className = `fullPerfSyllable ${syllToneClass(n)}`;
  badge.textContent = String(n);
...
if(state.currentSection === "Full"){
  ...
  if(state.fullPerfMode){
    ...
    blocks.forEach((b, blockIndex) => {
      ...
      for(const rawLine of body){
        ...
        appendFullPerfSyllableCount(boxes, t);
        sec.appendChild(row);
  ...
  blocks.forEach((b, idx) => {
    ...
    textWrap.className = "fullSectionTextWrap";
    ...
    autosizeTextarea(ta);
    const syllOverlay = document.createElement("div");
    syllOverlay.className = "fullSectionSyllableOverlay";
    textWrap.appendChild(ta);
    textWrap.appendChild(syllOverlay);
    refreshFullSectionSyllableOverlay(ta, syllOverlay);
  ...
  requestAnimationFrame(() => {
    try{
      fullWrap.querySelectorAll("textarea.fullSectionText").forEach((oneTa) => {
        autosizeTextarea(oneTa);
        const oneOverlay = oneTa.parentElement?.querySelector(".fullSectionSyllableOverlay");
        refreshFullSectionSyllableOverlay(oneTa, oneOverlay);
      });
    }catch{}
    try{
...
async function renderRecordings(){
  ...
  mine.forEach(rec => {
    ...
    play.textContent="▶";
    play.title="Play";
    play.dataset.audioPlayRecId = String(rec.id || "");
...
stop.addEventListener("click", () => {
  ...
  play.addEventListener("click", async () => {
    if(!rec.blob) return;
    await toggleAudioSyncFromRec(rec);
  });
...
async function startAudioSyncFromRec(rec){
  ...
  audio.onloadedmetadata = () => refreshAudioPlaybackUI();
  audio.ondurationchange = () => refreshAudioPlaybackUI();
  audio.onplay = () => { state.audioSyncPaused = false; refreshAudioPlaybackUI(); };
  audio.onpause = () => { if(state.audioSyncOn && !audio.ended){ state.audioSyncPaused = true; refreshAudioPlaybackUI(); } };
  audio.onended = () => {
    audioSyncStopInternal();
  };
...
function wireAudioProgressControl(){
  const bar = el.audioProgress;
  if(!bar || bar.dataset.wired === "1") return;
  bar.dataset.wired = "1";

  // IMPORTANT: Android/WebView has been failing to deliver the initial gesture
  // to the progress-bar element itself. Listen at WINDOW CAPTURE level and use
  // the finger coordinates to decide whether the gesture started on the bar.
  let gestureActive = false;
  let gestureKind = "";          // pointer | touch | mouse
  let gestureId = null;
  let lastClientX = 0;
  ...
  const activeAudio = () => {
    const a = state.audioSyncAudio;
    if(!state.audioSyncOn || !a) return null;
    if(!Number.isFinite(a.duration) || a.duration <= 0) return null;
    return a;
  };

  const barRect = () => {
    try{ return bar.getBoundingClientRect(); }catch{ return null; }
  };

  const pointIsOnBar = (clientX, clientY) => {
    const r = barRect();
    if(!r || r.width <= 0 || r.height <= 0) return false;
    // Give the visible bar a generous finger hit-zone on phones.
    const padY = 12;
    return Number(clientX) >= r.left && Number(clientX) <= r.right &&
           Number(clientY) >= (r.top - padY) && Number(clientY) <= (r.bottom + padY);
  };

  const timeFromClientX = (clientX) => {
    const a = activeAudio();
    const r = barRect();
    if(!a || !r || r.width <= 0) return 0;
    const ratio = clamp((Number(clientX) - r.left) / r.width, 0, 1);
  ...
  const paint = (target) => {
    const a = activeAudio();
    if(!a) return;
    const pct = clamp((target / a.duration) * 100, 0, 100);
    state.audioSeekPreviewSec = target;
    if(el.audioProgressFill) el.audioProgressFill.style.width = pct + "%";
    if(el.audioProgressThumb) el.audioProgressThumb.style.left = pct + "%";
    if(el.audioCurrentTime) el.audioCurrentTime.textContent = formatAudioTime(target);
    bar.setAttribute("aria-valuenow", String(target));
    bar.classList.add("isScrubbing");
  };

  const beginGesture = (kind, id, clientX, clientY, ev) => {
    const a = activeAudio();
    if(!a || committing || gestureActive) return false;
    if(!pointIsOnBar(clientX, clientY)) return false;
    ...
    gestureKind = kind;
    gestureId = id;
    lastClientX = Number(clientX);
    resumeAfterSeek = !a.paused && !state.audioSyncPaused;
    state.audioSeekDragging = true;

    // Pause immediately. If this executes, the user gets instant audible proof
    // that SRP captured the seek gesture.
    try{ if(!a.paused) a.pause(); }catch{}
    state.audioSyncPaused = true;
    if(state.audioSyncRaf){
      cancelAnimationFrame(state.audioSyncRaf);
    ...
    paint(timeFromClientX(lastClientX));
    return true;
  };

  const moveGesture = (kind, id, clientX, ev) => {
    if(!gestureActive || committing) return;
    if(kind !== gestureKind) return;
    if(gestureId != null && id != null && id !== gestureId) return;
    if(ev && ev.cancelable){ try{ ev.preventDefault(); }catch{} }
    lastClientX = Number(clientX);
    paint(timeFromClientX(lastClientX));
  };

  const endGesture = async (kind, id, clientX, ev) => {
    if(!gestureActive || committing) return;
    if(kind !== gestureKind) return;
    if(gestureId != null && id != null && id !== gestureId) return;

    if(ev && ev.cancelable){ try{ ev.preventDefault(); }catch{} }
    if(Number.isFinite(Number(clientX))) lastClientX = Number(clientX);

    const target = timeFromClientX(lastClientX);
    paint(target);
    ...
    try{
      await commitActiveAudioSeek(target, resumeAfterSeek);
    }finally{
      committing = false;
      gestureKind = "";
      gestureId = null;
      state.audioSeekDragging = false;
      state.audioSeekPreviewSec = null;
      bar.classList.remove("isScrubbing");
      refreshAudioPlaybackUI();
    }
  };

  // POINTER: capture phase at window level so overlays/rows cannot steal start.
  window.addEventListener("pointerdown", (e) => {
    beginGesture("pointer", e.pointerId, e.clientX, e.clientY, e);
  }, { capture:true, passive:false });

  window.addEventListener("pointermove", (e) => {
    moveGesture("pointer", e.pointerId, e.clientX, e);
  }, { capture:true, passive:false });

  window.addEventListener("pointerup", (e) => {
    endGesture("pointer", e.pointerId, e.clientX, e);
  }, { capture:true, passive:false });

  window.addEventListener("pointercancel", (e) => {
    endGesture("pointer", e.pointerId, lastClientX, e);
  }, { capture:true, passive:false });

  // TOUCH fallback for Samsung/Android WebViews that don't produce useful pointer events.
  window.addEventListener("touchstart", (e) => {
    if(gestureActive) return;
    const t = e.changedTouches && e.changedTouches[0];
    if(!t) return;
    beginGesture("touch", t.identifier, t.clientX, t.clientY, e);
  }, { capture:true, passive:false });

  window.addEventListener("touchmove", (e) => {
    if(!gestureActive || gestureKind !== "touch") return;
    const list = Array.from(e.touches || []);
    const t = list.find(x => gestureId == null || x.identifier === gestureId);
    if(!t) return;
    moveGesture("touch", t.identifier, t.clientX, e);
  }, { capture:true, passive:false });

  window.addEventListener("touchend", (e) => {
    if(!gestureActive || gestureKind !== "touch") return;
    const list = Array.from(e.changedTouches || []);
    const t = list.find(x => gestureId == null || x.identifier === gestureId);
    if(!t) return;
    endGesture("touch", t.identifier, t.clientX, e);
  }, { capture:true, passive:false });

  window.addEventListener("touchcancel", (e) => {
    if(!gestureActive || gestureKind !== "touch") return;
    endGesture("touch", gestureId, lastClientX, e);
  }, { capture:true, passive:false });

  // Mouse fallback for desktop testing.
  window.addEventListener("mousedown", (e) => {
    if(gestureActive) return;
    beginGesture("mouse", 1, e.clientX, e.clientY, e);
  }, { capture:true, passive:false });
  window.addEventListener("mousemove", (e) => {
    moveGesture("mouse", 1, e.clientX, e);
  }, { capture:true, passive:false });
  window.addEventListener("mouseup", (e) => {
    endGesture("mouse", 1, e.clientX, e);
  }, { capture:true, passive:false });

  // Final tap/click fallback. This also uses coordinates, not event.target.
  window.addEventListener("click", async (e) => {
    if(Date.now() < suppressClickUntil || gestureActive || committing) return;
    const a = activeAudio();
    if(!a || !pointIsOnBar(e.clientX, e.clientY)) return;
    if(e.cancelable){ try{ e.preventDefault(); }catch{} }
    const target = timeFromClientX(e.clientX);
    paint(target);
    const wasPlaying = !a.paused && !state.audioSyncPaused;
    await commitActiveAudioSeek(target, wasPlaying);
    bar.classList.remove("isScrubbing");
  }, { capture:true, passive:false });
  ...
  bar.addEventListener("keydown", async (e) => {
    ...
    let target = Number(a.currentTime) || 0;
    if(e.key === "ArrowLeft") target -= 5;
    else if(e.key === "ArrowRight") target += 5;
    else if(e.key === "Home") target = 0;
    else if(e.key === "End") target = a.duration;
    else return;
    e.preventDefault();
    const wasPlaying = !a.paused && !state.audioSyncPaused;
    await commitActiveAudioSeek(clamp(target, 0, a.duration), wasPlaying);
  });
}
...
async function resolveNotesImageElementSources(){
  ...
  for(const img of imgs){
    ...
    // Never resolve an image merely because the blob exists in IndexedDB. The
    // image must belong to this project's manifest; otherwise a stale foreign
    // image id could display inside the wrong project's Notes page.
    if(!entry || entry.deleted || String(entry.projectId || state.project?.id || "") !== String(state.project?.id || "")){
      block?.remove();
      continue;
    }
    ...
    try{
      ...
      if(rec?.blob && String(rec.projectId || "") === String(state.project?.id || "")){
...
function saveNotesDoc(opts={}){
  if(!state.project || !el.notesEditor) return false;

  const currentProjectId = String(state.project.id || "");
  const boundProjectId = String(state.notesEditorProjectId || "");

  // Hard project boundary: never allow stale hidden Notes DOM from one project
  // to be written into another project. This is the main cross-project leak fix.
  if(!boundProjectId || boundProjectId !== currentProjectId) return false;

  normalizeNotesExplicitColors(state.project.notesColor || state.notesDefaultColor || "#151515");
  syncProjectNotesImagesFromEditor();

  let newDoc = normalizeNotesHtml(sanitizeNotesHtml(serializeNotesEditorHtml()));
  const newHasNotes = notesDocHasMeaningfulContent(newDoc);
  const allowBlank = !!opts.allowBlank;
  ...
  // Preserve real notes only for non-user/background blank saves. A deliberate
  // editor mutation is allowed to clear the document completely.
  if(!allowBlank && !newHasNotes && (prevHasNotes || backupHasNotes)){
    const bestNotes = mergeNotesSafely(
      { notesDoc: prevDoc, notesColor: state.project.notesColor, notesUpdatedAt: Number(state.project.notesUpdatedAt) || 0, notesClearedAt: Number(state.project.notesClearedAt) || 0, updatedAt: Number(state.project.notesUpdatedAt) || 0 },
      backup
    );
    if(bestNotes){
      state.project.notesDoc = String(bestNotes.notesDoc || "");
  ...
  // Canonical empty state: don't keep harmless browser scaffolding such as
  // <div><br></div> after the user has deleted everything.
  if(!newHasNotes) newDoc = "";

  const stamp = now();
  state.project.notesDoc = newDoc;
  state.project.notesUpdatedAt = stamp;
  state.project.notesClearedAt = newHasNotes ? 0 : stamp;
  state.project.updatedAt = stamp;
  if(!state.project.notesColor) state.project.notesColor = state.notesDefaultColor || "#151515";

  writeNotesBackup(state.project, { allowBlankOverwrite: allowBlank && !newHasNotes });
  upsertProject(state.project);
  return true;
}
...
function persistActiveProjectState(opts={}){
  ...
  const requestedSave = (opts && typeof opts.saveNotes === "boolean")
    ? opts.saveNotes
    : true;
  const editorOwnsCurrentProject = !!(
    state.notesOpen &&
    el.notesEditor &&
    String(state.notesEditorProjectId || "") === String(state.project.id || "")
  );

  if(requestedSave && editorOwnsCurrentProject){
    saveNotesDoc({ allowBlank:true });
    return;
  }
...
function renderNotesFromProject(){
  if(!el.notesEditor || !state.project) return;
  state.notesEditorProjectId = String(state.project.id || "");
...
function closeNotesModal(){
  ...
  const ownsCurrentProject = !!(
    state.notesOpen &&
    el.notesEditor &&
    state.project &&
    String(state.notesEditorProjectId || "") === String(state.project.id || "")
  );
  if(ownsCurrentProject) saveNotesDoc({ allowBlank:true });
  ...
  // Do not leave another project's Notes HTML sitting invisibly in the DOM.
  if(el.notesEditor) el.notesEditor.innerHTML = "";
  state.notesEditorProjectId = "";
  state.notesLastRange = null;
  clearNotesImageSelection();
}
...
if(el.capoInput){
  ...
  if(el.notesEditor){
    ...
    el.notesEditor.addEventListener("blur", () => {
      cacheNotesSelection();
      if(state.notesOpen) saveNotesDoc({ allowBlank:true });
    });
  }
  ...
  const flushNotesPersistence = () => {
    try{
      if(state.project){
        if(state.notesOpen && el.notesEditor && String(state.notesEditorProjectId || "") === String(state.project.id || "")) saveNotesDoc({ allowBlank:true });
