...
function refreshFullSectionSyllableOverlay(ta, overlay){
  ...
  const padLeft = parseFloat(cs?.paddingLeft || "4") || 4;
  const padTop = parseFloat(cs?.paddingTop || "0") || 0;
  const padRight = parseFloat(cs?.paddingRight || "4") || 4;

  let ctx = null;
  try{
    const canvas = refreshFullSectionSyllableOverlay._canvas || (refreshFullSectionSyllableOverlay._canvas = document.createElement("canvas"));
  ...
  const maxLeft = Math.max(padLeft + 8, (ta.clientWidth || 0) - padRight - 28);
  lines.forEach((raw, i) => {
    ...
    let width = 0;
    try{ width = ctx ? ctx.measureText(String(raw || "").replace(/\s+$/g, "")).width : 0; }catch{}
    const left = Math.min(maxLeft, Math.max(padLeft + 8, padLeft + width + 8));
    badge.style.left = `${Math.round(left)}px`;
    badge.style.top = `${Math.round(padTop + i * lineHeight)}px`;
