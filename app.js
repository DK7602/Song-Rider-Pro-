...
function updateAudioProgressUI(){
  ...
  if(!active){
    ...
    if(el.audioProgress){
      ...
      el.audioProgress.disabled = true;
    }
  ...
  if(el.audioProgress){
    el.audioProgress.max = duration > 0 ? String(duration) : "1";
    // IMPORTANT: while the user is scrubbing, do not overwrite the thumb from the RAF.
    if(!state.audioSeekDragging){
      el.audioProgress.value = duration > 0 ? String(actualCurrent) : "0";
    }
    el.audioProgress.disabled = !(duration > 0);
  }
...
function wireAudioProgressControl(){
  if(!el.audioProgress || el.audioProgress.dataset.wired === "1") return;
  el.audioProgress.dataset.wired = "1";

  const readSlider = () => {
    const max = Number(el.audioProgress.max) || 0;
    const raw = Number(el.audioProgress.value) || 0;
    return max > 0 ? clamp(raw, 0, max) : Math.max(0, raw);
  };

  const beginScrub = () => {
    if(el.audioProgress.disabled) return;
    state.audioSeekDragging = true;
    state.audioSeekPreviewSec = readSlider();
  };

  const scrub = () => {
    if(el.audioProgress.disabled) return;
    if(!state.audioSeekDragging) state.audioSeekDragging = true;
  ...
  const finishScrub = () => {
    if(el.audioProgress.disabled) return;
    const target = readSlider();
    // Commit exactly where the thumb was released.
    seekActiveAudioTo(target, { scrubbing:true });
    state.audioSeekDragging = false;
    state.audioSeekPreviewSec = null;

    // Let the media element finish its seek before normal playback UI resumes.
    requestAnimationFrame(() => refreshAudioPlaybackUI());
    setTimeout(() => refreshAudioPlaybackUI(), 80);
  };

  el.audioProgress.addEventListener("pointerdown", beginScrub);
  el.audioProgress.addEventListener("touchstart", beginScrub, { passive:true });
  el.audioProgress.addEventListener("input", scrub);
  el.audioProgress.addEventListener("change", finishScrub);
  el.audioProgress.addEventListener("pointerup", finishScrub);
  el.audioProgress.addEventListener("pointercancel", finishScrub);
  el.audioProgress.addEventListener("touchend", finishScrub, { passive:true });
}
