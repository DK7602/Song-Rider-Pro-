...
function updateAudioProgressUI(){
  ...
  if(el.audioProgress){
    ...
    el.audioProgress.value = duration > 0 ? String(current) : "0";
...
function seekActiveAudioTo(sec){
  const a = state.audioSyncAudio;
...
function wireAudioProgressControl(){
  ...
  const seek = () => seekActiveAudioTo(Number(el.audioProgress.value) || 0);
  el.audioProgress.addEventListener("input", seek);
  el.audioProgress.addEventListener("change", seek);
}
