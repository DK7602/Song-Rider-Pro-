...
function serializeNotesEditorHtml(){
  ...
  clone.querySelectorAll('.notesImageBlock.is-selected').forEach((node) => node.classList.remove('is-selected'));
...
function startNotesImageResize(block, startEvent){
  if(!block || !el?.notesEditor) return;
  const editorRect = el.notesEditor.getBoundingClientRect();
  const startWidth = Math.max(140, Math.round(block.getBoundingClientRect().width || 0));
  state.notesImageResize = {
    id: String(block.getAttribute('data-notes-image-id') || ''),
    block,
    startX: Number(startEvent?.clientX || 0),
    startWidth,
    minWidth: 140,
    maxWidth: Math.max(180, Math.floor(editorRect.width || window.innerWidth || 720))
  };
  block.classList.add('is-selected');
}

function moveNotesImageResize(ev){
  const rs = state.notesImageResize;
  if(!rs?.block) return;
  const dx = Number(ev?.clientX || 0) - rs.startX;
  const next = clamp(Math.round(rs.startWidth + dx), rs.minWidth, rs.maxWidth);
  rs.block.style.width = `${next}px`;
}

function stopNotesImageResize(commit=true){
  const rs = state.notesImageResize;
  if(!rs) return;
...
async function removeSelectedNotesImageBlock(){
...
function getPublicNoteImageUrl(path){
  try{
    ...
    if(cloud?.client?.storage?.from){
      const out = cloud.client.storage.from(CLOUD_AUDIO_BUCKET).getPublicUrl(p);
      return String(out?.data?.publicUrl || '').trim();
    }
    const base = String(SUPABASE_URL || '').replace(/\/$/, '');
    return p ? `${base}/storage/v1/object/public/${CLOUD_AUDIO_BUCKET}/${p}` : '';
  }catch{
    return '';
  }
}
...
async function insertNotesImageFromFile(file){
  ...
  restoreNotesSelection();
  const html = `<div class="notesImageBlock is-loading" contenteditable="false" data-notes-image-id="${imageId}" style="width:min(100%,720px);"><img class="notesImage" data-notes-image-id="${imageId}" alt="${escapeHtml(file.name || 'Photo')}" loading="lazy" decoding="async"><span class="notesImageResizeHandle" contenteditable="false" aria-hidden="true"></span></div><div><br></div>`;
  try{
  ...
  saveNotesDoc();
  pushNotesHistorySnapshot(true);
  if(cloud.ready && cloud.client && getAuthUserId()) queueCloudSync('notes-image-add');
  return true;
}
...
function renderNotesFromProject(){
  ...
  normalizeNotesExplicitColors(state?.project?.notesColor || state.notesDefaultColor || "#151515");
  Array.from(el.notesEditor.querySelectorAll('.notesImageBlock')).forEach(ensureNotesImageBlockChrome);
  clearNotesImageSelection();
  resolveNotesImageElementSources();
  applyNotesEditorDefaults();
}
...
if(el.capoInput){
  ...
  if(el.notesEditor){
    ...
    el.notesEditor.addEventListener("pointerdown", (e) => {
      const handle = e.target?.closest?.('.notesImageResizeHandle');
      if(handle){
        const block = handle.closest('.notesImageBlock');
        if(block){
          e.preventDefault();
          e.stopPropagation();
  ...
  document.addEventListener("pointermove", (e) => {
    if(state.notesImageResize) moveNotesImageResize(e);
  }, { passive:false });

  document.addEventListener("pointerup", () => {
    if(state.notesImageResize) stopNotesImageResize(true);
  }, true);
  document.addEventListener("pointercancel", () => {
    if(state.notesImageResize) stopNotesImageResize(true);
  }, true);
