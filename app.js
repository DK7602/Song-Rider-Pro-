...
function drumRim(vel=1){
  ...
  // Replaced old plinky rimshot with a short clap/percussion noise hit.
  const out = ctx.createGain();
  out.gain.setValueAtTime(0.0001, t0);
  out.gain.exponentialRampToValueAtTime(0.26 * v, t0 + 0.0015);
  out.gain.exponentialRampToValueAtTime(0.0001, t0 + 0.09);

  const src = ctx.createBufferSource();
  src.buffer = makeNoiseBuffer(ctx, 0.10);
  ...
  g1.gain.exponentialRampToValueAtTime(0.20 * v, t0 + 0.001);
  g1.gain.exponentialRampToValueAtTime(0.0001, t0 + 0.020);

  const g2 = ctx.createGain();
  g2.gain.setValueAtTime(0.0001, t0 + 0.010);
  g2.gain.exponentialRampToValueAtTime(0.13 * v, t0 + 0.013);
  g2.gain.exponentialRampToValueAtTime(0.0001, t0 + 0.040);

  const g3 = ctx.createGain();
  g3.gain.setValueAtTime(0.0001, t0 + 0.022);
  g3.gain.exponentialRampToValueAtTime(0.10 * v, t0 + 0.026);
  g3.gain.exponentialRampToValueAtTime(0.0001, t0 + 0.070);
  ...
  scheduleCleanup([out,src,hp,bp,tone,g1,g2,g3,lp], 220);
}
...
function drumHatClosed(vel=1){
  ...
  const out = ctx.createGain();
  out.gain.setValueAtTime(0.0001, t0);
  out.gain.exponentialRampToValueAtTime(0.19 * v, t0 + 0.0016);
  out.gain.exponentialRampToValueAtTime(0.0001, t0 + 0.040);
  ...
  // smoother top shimmer; removes the old plinky / rim-like overtone
  const shimmerFreqs = [5600, 7200, 9100];
  const shimmerMix = ctx.createGain();
  shimmerMix.gain.value = 0.012 * v;

  const oscs = [];
  for(const f of shimmerFreqs){
    const o = ctx.createOscillator();
    o.type = "triangle";
    o.frequency.setValueAtTime(f * (1 + (Math.random()*2 - 1) * 0.0012), t0);
    oscs.push(o);
    o.connect(shimmerMix);
    o.start(t0);
    o.stop(t0 + 0.028);
  }
  ...
  nG.gain.exponentialRampToValueAtTime(0.17 * v, t0 + 0.0016);
  ...
  scheduleCleanup([out,ns,hp,bp,lp,nG,shimmerMix, ...oscs], 220);
}
...
function startDrums(){
  ...
  const runDrumStep = () => {
    ...
    if(state.drumStyle === "rap"){
      if(s === 0 || s === 6 || s === 10) drumHit("kick", s === 0 ? 1.0 : 0.76);
      if(s === 4 || s === 12) drumHit("snare", 0.94);
      if(s === 2 || s === 6 || s === 10 || s === 14) drumHit("hat", 0.56);
      if(s === 7 || s === 15) drumHit("openhat", 0.24);
      if(s === 3 || s === 11) drumHit("rim", 0.24);
    } else if(state.drumStyle === "rock"){
      if(s === 0 || s === 8 || s === 10) drumHit("kick", s === 0 ? 0.98 : 0.82);
      if(s === 4 || s === 12) drumHit("snare", 1.0);
      if(s % 2 === 0) drumHit("hat", 0.74);
      if(s === 15) drumHit("openhat", 0.40);
      if(s === 14) drumHit("rim", 0.16);
    } else if(state.drumStyle === "hardrock"){
      if(s === 0 || s === 3 || s === 6 || s === 8 || s === 10 || s === 14) drumHit("kick", s === 0 || s === 8 ? 1.0 : 0.82);
      if(s === 4 || s === 12) drumHit("snare", 1.03);
      if(s % 2 === 0) drumHit("hat", 0.80);
      if(s === 15) drumHit("openhat", 0.46);
      if(s === 13) drumHit("tommid", 0.26);
    } else if(state.drumStyle === "pop"){
      if(s === 0 || s === 7 || s === 8 || s === 14) drumHit("kick", s === 0 || s === 8 ? 0.98 : 0.76);
      if(s === 4 || s === 12) drumHit("snare", 0.98);
      if(s % 2 === 0) drumHit("hat", s === 14 ? 0.86 : 0.66);
      if(s % 4 === 1 || s % 4 === 3) drumHit("shaker", 0.28);
      if(s === 15) drumHit("openhat", 0.26);
    } else if(state.drumStyle === "modernrock"){
      if(s === 0 || s === 5 || s === 8 || s === 11) drumHit("kick", s === 0 || s === 8 ? 1.0 : 0.84);
      if(s === 4 || s === 12) drumHit("snare", 1.02);
      if(s % 2 === 0) drumHit("hat", 0.80);
      if(s === 3 || s === 11) drumHit("shaker", 0.18);
      if(s === 15) drumHit("openhat", 0.50);
      if(s === 14) drumHit("tomhi", 0.24);
    } else if(state.drumStyle === "popbounce"){
      if(s === 0 || s === 6 || s === 8 || s === 14) drumHit("kick", s === 0 || s === 8 ? 0.96 : 0.74);
      if(s === 4 || s === 12) drumHit("snare", 0.94);
      if(s % 4 === 0 || s % 4 === 3) drumHit("hat", 0.70);
      if(s % 2 === 1) drumHit("shaker", 0.34);
      if(s === 7 || s === 15) drumHit("rim", 0.22);
      if(s === 15) drumHit("openhat", 0.24);
    } else if(state.drumStyle === "trapphantom"){
      if(s === 0 || s === 3 || s === 8 || s === 10 || s === 14) drumHit("kick", s === 0 || s === 8 ? 1.0 : 0.76);
      if(s === 4 || s === 12) drumHit("snare", 0.98);
      if(s === 1 || s === 2 || s === 3 || s === 5 || s === 6 || s === 7 || s === 9 || s === 10 || s === 11 || s === 13 || s === 14 || s === 15){
        drumHit("hat", (s % 2 === 0) ? 0.44 : 0.34);
      }
      if(s === 6 || s === 14) drumHit("rim", 0.28);
      if(s === 15) drumHit("openhat", 0.22);
    } else if(state.drumStyle === "altpop"){
      if(s === 0 || s === 5 || s === 8 || s === 13) drumHit("kick", s === 0 || s === 8 ? 0.95 : 0.74);
      if(s === 4 || s === 12) drumHit("snare", 0.96);
      if(s % 2 === 0 || s === 11 || s === 15) drumHit("hat", 0.72);
      if(s % 2 === 1) drumHit("shaker", 0.26);
      if(s === 14) drumHit("rim", 0.22);
      if(s === 15) drumHit("openhat", 0.32);
      if(s === 13) drumHit("tomhi", 0.22);
    } else {
      if(s === 0 || s === 7 || s === 8) drumHit("kick", 0.98);
      if(s === 4 || s === 12) drumHit("snare", 0.98);
