...
function parseFullTextToSectionCards(fullText){
  ...
  if(hasBreaks){
    ...
    let usedCurrentHeadingSlot = false;
    ...
    for(let i=0; i<lines.length; i++){
      ...
      if(heading){
        ...
        const curBucket = buckets[cur] || [];
        const curHasCards = Array.isArray(curBucket) && curBucket.length > 0;
        const curHasTitle = !!String(titles[cur] || "").trim();

        // ✅ Use the already-created first page for the first detected heading.
        // The previous logic always allocated the NEXT page on the first heading,
        // which made imported card pages start late and appear out of order.
        if(!usedCurrentHeadingSlot && !curHasCards && !curHasTitle){
          titles[cur] = String(heading || "").trim();
          usedCurrentHeadingSlot = true;
          ensureBucket(cur);
          continue;
        }
...
ta.value = state.project.fullText || "";
  ...
  // ✅ On first open: optionally seed sections from Full text ONCE.
  // Keep this light: apply once, save once, and only re-render the current sheet.
...
function isSectionVisible(sec){
  ...
  if(String(state.project?.sectionTitles?.[sec] || "").trim()) return true; // titled pages from Full import should stay visible
