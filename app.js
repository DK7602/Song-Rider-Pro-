...
function hydrateProjectsArrayWithNotes(projects, scope){
  ...
  const out = arr.map((proj) => {
    ...
    try{
      const guestBackup = parseNotesBackupRaw(localStorage.getItem(notesBackupKey(projectId, "guest")));
      if(guestBackup) candidates.push(guestBackup);
    }catch{}

    try{
      const authId = String(getAuthUserId() || "").trim();
      if(authId){
        const authBackup = parseNotesBackupRaw(localStorage.getItem(notesBackupKey(projectId, authId)));
        if(authBackup) candidates.push(authBackup);
      }
    }catch{}
...
function wireModalInnerScroller(root, scroller){
  if(!root || !scroller || root.dataset.modalScrollWired === '1') return;
  root.dataset.modalScrollWired = '1';

  const stopIfInside = (e) => {
    if(e.target && e.target.closest && e.target.closest('.archiveList, .archiveAddList, .archiveCard, .archiveMoveBox')) return;
    e.preventDefault();
  };

  root.addEventListener('touchmove', stopIfInside, { passive:false });
...
const v = Number.isFinite(raw) ? raw : 0; 
  ...
  wireModalInnerScroller(el.archiveModal, el.archiveAddList || el.archiveList);
