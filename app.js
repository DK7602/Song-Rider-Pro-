...
function serializeNotesEditorHtml(){
  ...
  clone.querySelectorAll('.notesImageResizeHandle,.notesImageDeleteBtn,.notesImageMoveUpBtn,.notesImageMoveDownBtn').forEach((node) => node.remove());
...
function startNotesImageDrag(block, startEvent){
  ...
  drag.docMove = (ev) => {
    if(state.notesImageResize?.block) return;
    moveNotesImageDrag(ev);
  };
  drag.docUp = () => {
  ...
  drag.holdTimer = setTimeout(() => {
    ...
    live.active = true;
    live.activationDx = Number(live.lastX || 0) - Number(live.startX || 0);
    live.activationDy = Number(live.lastY || 0) - Number(live.startY || 0);
...
function moveNotesImageDrag(ev){
  ...
  if(!drag.active){
    if(Math.abs(dxFromStart) > 24 || Math.abs(dyFromStart) > 24){
  ...
  const translateY = dyFromStart - Number(drag.activationDy || 0);
  drag.block.style.transform = `translateY(${Math.round(translateY)}px) scale(0.985)`;

  const editor = el?.notesEditor;
  if(!editor) return;
  const blocks = Array.from(editor.querySelectorAll('.notesImageBlock')).filter((node) => node !== drag.block);
  ...
  const parent = drag.block.parentNode;
  if(!parent) return;
  const isSpacer = (node) => !!(node && node.nodeType === 1 && String(node.innerHTML || '').trim().toLowerCase() === '<br>');
  const nextElem = (node) => {
    let cur = node?.nextSibling || null;
  ...
  let spacerAfter = nextElem(drag.block);
  if(!isSpacer(spacerAfter)) spacerAfter = null;

  if(insertBefore){
    if(insertBefore !== drag.block && insertBefore !== spacerAfter){
      parent.insertBefore(drag.block, insertBefore);
      if(spacerAfter) parent.insertBefore(spacerAfter, drag.block.nextSibling);
      drag.didReorder = true;
    }
  }else{
    const lastEl = parent.lastElementChild;
    if(lastEl && lastEl !== drag.block && lastEl !== spacerAfter){
      parent.appendChild(drag.block);
      if(spacerAfter) parent.insertBefore(spacerAfter, drag.block.nextSibling);
      drag.didReorder = true;
    }
  }
...
function stopNotesImageDrag(){
  ...
  if(drag?.active && drag?.didReorder){
    saveNotesDoc();
    pushNotesHistorySnapshot(true);
    if(cloud.ready && cloud.client && getAuthUserId()) triggerImmediateCloudSync('notes-image-move');
  }
...
if(el.capoInput){
  ...
  if(el.notesEditor){
    ...
    el.notesEditor.addEventListener('pointerdown', (e) => {
      ...
      const moveBtn = e.target?.closest?.('.notesImageDeleteBtn');
      if(moveBtn) return;
      const imageBlock = e.target?.closest?.('.notesImageBlock');
      if(imageBlock){
        e.preventDefault();
