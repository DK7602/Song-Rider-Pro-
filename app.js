...
function drumHatClosed(vel=1){
  ...
  metalMix.gain.value = 0.05 * v;
  const freqs = [4300, 5600, 7100, 8900, 10300];
  const oscs = [];
...
function drumHatOpen(vel=1){
  ...
  const metalMix = ctx.createGain();
  metalMix.gain.value = 0.04 * v;
  const freqs = [3950, 5100, 6800, 8200, 9600];
  const oscs = [];
  for(const f of freqs){
    const o = ctx.createOscillator();
    o.type = "square";
...
// --- Distinct acoustic / keyboard / synth / EP voices ---
const studioOakGrandNote = makeVariantKeyNote(grandPianoNote, {
  hp: 34, bodyFreq: 225, bodyGain: 3.0, presenceFreq: 2050, presenceGain: 0.5, lp: 5400, roomMix: 0.12, outGain: 1.05
});

function skylineStageKeysNote(ctx, freq, durMs, vel=0.84){
  const t0 = ctx.currentTime;
  const dur = Math.max(0.10, durMs/1000);
  const f0 = clamp(freq, 70, 2600);
  const v = clamp(vel, 0.15, 1.0);
  ...
  const tri = ctx.createOscillator(); tri.type = 'triangle'; tri.frequency.value = f0 * 1.002;
  const sub = ctx.createOscillator(); sub.type = 'sine'; sub.frequency.value = Math.max(45, f0 * 0.5);
  ...
  env.gain.setValueAtTime(peak * 0.86, t0 + 0.09);
  const endTime = t0 + dur;
  env.gain.setValueAtTime(peak * 0.72, endTime);
  const tail = clamp(0.70 + dur * 0.34, 0.95, 2.90);
  env.gain.exponentialRampToValueAtTime(0.0001, endTime + tail);

  const hp = ctx.createBiquadFilter(); hp.type = 'highpass'; hp.frequency.value = 140;
  const body = ctx.createBiquadFilter(); body.type = 'peaking'; body.frequency.value = 720; body.Q.value = 0.9; body.gain.value = 1.6;
  const presence = ctx.createBiquadFilter(); presence.type = 'peaking'; presence.frequency.value = 2800; presence.Q.value = 0.9; presence.gain.value = 2.8;
  const lp = ctx.createBiquadFilter(); lp.type = 'lowpass'; lp.frequency.value = 6200; lp.Q.value = 0.7;
  const chorus = ctx.createDelay(0.05); chorus.delayTime.value = 0.009;
  const chorusGain = ctx.createGain(); chorusGain.gain.value = 0.18;
  const room = makeSoftRoom(ctx);
  const dry = ctx.createGain(); dry.gain.value = 0.82;
  const wet = ctx.createGain(); wet.gain.value = 0.07;
  const outTrim = ctx.createGain(); outTrim.gain.value = 0.78;

  saw.connect(g1); g1.connect(mix);
  tri.connect(g2); g2.connect(mix);
  sub.connect(g3); g3.connect(mix);
  mix.connect(hp); hp.connect(body); body.connect(presence); presence.connect(lp); lp.connect(env);
  env.connect(dry); dry.connect(outTrim);
  env.connect(chorus); chorus.connect(chorusGain); chorusGain.connect(outTrim);
  env.connect(room.in); room.wet.connect(wet); wet.connect(outTrim);

  saw.start(t0); tri.start(t0); sub.start(t0);
  const stopAt = endTime + tail + 0.08;
  saw.stop(stopAt); tri.stop(stopAt); sub.stop(stopAt);

  scheduleCleanup([saw,tri,sub,g1,g2,g3,mix,env,hp,body,presence,lp,chorus,chorusGain,dry,wet,outTrim, ...room.nodes], durMs + tail*1000 + 1000);
  return { out: outTrim, nodes:[saw,tri,sub,g1,g2,g3,mix,env,hp,body,presence,lp,chorus,chorusGain,dry,wet,outTrim, ...room.nodes] };
}
...
function glassLanternNote(ctx, freq, durMs, vel=0.80){
  const t0 = ctx.currentTime;
  const dur = Math.max(0.10, durMs/1000);
  const f0 = clamp(freq, 90, 2600);
  const v = clamp(vel, 0.12, 1.0);

  const car = ctx.createOscillator(); car.type = 'sine'; car.frequency.value = f0;
  const mod = ctx.createOscillator(); mod.type = 'sine'; mod.frequency.value = f0 * 5.2;
  const shimmer = ctx.createOscillator(); shimmer.type = 'triangle'; shimmer.frequency.value = f0 * 2.98;
  const modGain = ctx.createGain();
  modGain.gain.setValueAtTime(f0 * 2.1, t0);
  modGain.gain.exponentialRampToValueAtTime(f0 * 0.12, t0 + 0.16);
  mod.connect(modGain); modGain.connect(car.frequency);

  const carG = ctx.createGain(); carG.gain.value = 0.42;
  const shimmerG = ctx.createGain(); shimmerG.gain.value = 0.18;
  const mix = ctx.createGain();
  ...
  env.gain.setValueAtTime(peak * 0.72, t0 + 0.05);
  const endTime = t0 + dur;
  env.gain.setValueAtTime(peak * 0.48, endTime);
  const tail = clamp(1.10 + dur * 0.42, 1.30, 3.40);
  env.gain.exponentialRampToValueAtTime(0.0001, endTime + tail);

  const hp = ctx.createBiquadFilter(); hp.type = 'highpass'; hp.frequency.value = 220;
  const bell = ctx.createBiquadFilter(); bell.type = 'peaking'; bell.frequency.value = 2550; bell.Q.value = 1.1; bell.gain.value = 4.5;
  const air = ctx.createBiquadFilter(); air.type = 'peaking'; air.frequency.value = 6200; air.Q.value = 1.0; air.gain.value = 3.2;
  const lp = ctx.createBiquadFilter(); lp.type = 'lowpass'; lp.frequency.value = 9800; lp.Q.value = 0.7;
  const room = makeSoftRoom(ctx);
  const dry = ctx.createGain(); dry.gain.value = 0.70;
  const wet = ctx.createGain(); wet.gain.value = 0.18;
  const outTrim = ctx.createGain(); outTrim.gain.value = 0.60;

  car.connect(carG); carG.connect(mix);
  shimmer.connect(shimmerG); shimmerG.connect(mix);
  mix.connect(hp); hp.connect(bell); bell.connect(air); air.connect(lp); lp.connect(env);
  env.connect(dry); dry.connect(outTrim);
  env.connect(room.in); room.wet.connect(wet); wet.connect(outTrim);

  car.start(t0); mod.start(t0); shimmer.start(t0);
  const stopAt = endTime + tail + 0.08;
  car.stop(stopAt); mod.stop(stopAt); shimmer.stop(stopAt);

  scheduleCleanup([car,mod,shimmer,modGain,carG,shimmerG,mix,env,hp,bell,air,lp,dry,wet,outTrim, ...room.nodes], durMs + tail*1000 + 1000);
  return { out: outTrim, nodes:[car,mod,shimmer,modGain,carG,shimmerG,mix,env,hp,bell,air,lp,dry,wet,outTrim, ...room.nodes] };
}

function prismPopNote(ctx, freq, durMs, vel=0.82){
  const t0 = ctx.currentTime;
  const dur = Math.max(0.10, durMs/1000);
  const f0 = clamp(freq, 80, 2400);
  const v = clamp(vel, 0.15, 1.0);

  const pulse = ctx.createOscillator(); pulse.type = 'square'; pulse.frequency.value = f0;
  const sine = ctx.createOscillator(); sine.type = 'sine'; sine.frequency.value = f0 * 2.0;
  const g1 = ctx.createGain(); g1.gain.value = 0.17;
  ...
  const peak = 0.044 * v;
  env.gain.setValueAtTime(0.0001, t0);
  env.gain.linearRampToValueAtTime(peak, t0 + 0.003);
  env.gain.setValueAtTime(peak * 0.64, t0 + 0.03);
  const endTime = t0 + dur;
  env.gain.setValueAtTime(peak * 0.34, endTime);
  const tail = clamp(0.35 + dur * 0.22, 0.45, 1.45);
  env.gain.exponentialRampToValueAtTime(0.0001, endTime + tail);

  const hp = ctx.createBiquadFilter(); hp.type = 'highpass'; hp.frequency.value = 260;
  const body = ctx.createBiquadFilter(); body.type = 'peaking'; body.frequency.value = 1100; body.Q.value = 0.9; body.gain.value = 2.2;
  const air = ctx.createBiquadFilter(); air.type = 'peaking'; air.frequency.value = 4300; air.Q.value = 1.0; air.gain.value = 3.8;
  const lp = ctx.createBiquadFilter(); lp.type = 'lowpass'; lp.frequency.value = 7200; lp.Q.value = 0.7;
  const slap = ctx.createDelay(0.03); slap.delayTime.value = 0.006;
  const slapG = ctx.createGain(); slapG.gain.value = 0.11;
  const outTrim = ctx.createGain(); outTrim.gain.value = 0.70;

  pulse.connect(g1); g1.connect(mix);
  sine.connect(g2); g2.connect(mix);
  mix.connect(hp); hp.connect(body); body.connect(air); air.connect(lp); lp.connect(env);
  env.connect(outTrim);
  env.connect(slap); slap.connect(slapG); slapG.connect(outTrim);

  pulse.start(t0); sine.start(t0);
  const stopAt = endTime + tail + 0.06;
  pulse.stop(stopAt); sine.stop(stopAt);

  scheduleCleanup([pulse,sine,g1,g2,mix,env,hp,body,air,lp,slap,slapG,outTrim], durMs + tail*1000 + 800);
  return { out: outTrim, nodes:[pulse,sine,g1,g2,mix,env,hp,body,air,lp,slap,slapG,outTrim] };
}
...
function midnightGlassNote(ctx, freq, durMs, vel=0.78){
  const t0 = ctx.currentTime;
  const dur = Math.max(0.10, durMs/1000);
  const f0 = clamp(freq, 90, 2600);
  const v = clamp(vel, 0.12, 1.0);

  const s1 = ctx.createOscillator(); s1.type = 'sine'; s1.frequency.value = f0;
  const s2 = ctx.createOscillator(); s2.type = 'sine'; s2.frequency.value = f0 * 2.01;
  const s3 = ctx.createOscillator(); s3.type = 'triangle'; s3.frequency.value = f0 * 3.0;
  const g1 = ctx.createGain(); g1.gain.value = 0.34;
  const g2 = ctx.createGain(); g2.gain.value = 0.16;
  ...
  const peak = 0.040 * v;
  env.gain.setValueAtTime(0.0001, t0);
  env.gain.linearRampToValueAtTime(peak, t0 + 0.018);
  env.gain.setValueAtTime(peak * 0.86, t0 + 0.12);
  const endTime = t0 + dur;
  env.gain.setValueAtTime(peak * 0.62, endTime);
  const tail = clamp(1.60 + dur * 0.48, 1.90, 4.20);
  env.gain.exponentialRampToValueAtTime(0.0001, endTime + tail);

  const hp = ctx.createBiquadFilter(); hp.type = 'highpass'; hp.frequency.value = 180;
  const glass = ctx.createBiquadFilter(); glass.type = 'peaking'; glass.frequency.value = 3600; glass.Q.value = 1.2; glass.gain.value = 2.6;
  const dark = ctx.createBiquadFilter(); dark.type = 'lowpass'; dark.frequency.value = 7200; dark.Q.value = 0.8;
  const room = makeSoftRoom(ctx);
  const dry = ctx.createGain(); dry.gain.value = 0.66;
  const wet = ctx.createGain(); wet.gain.value = 0.22;
  const outTrim = ctx.createGain(); outTrim.gain.value = 0.56;

  s1.connect(g1); g1.connect(mix);
  s2.connect(g2); g2.connect(mix);
  s3.connect(g3); g3.connect(mix);
  mix.connect(hp); hp.connect(glass); glass.connect(dark); dark.connect(env);
  env.connect(dry); dry.connect(outTrim);
  env.connect(room.in); room.wet.connect(wet); wet.connect(outTrim);

  s1.start(t0); s2.start(t0); s3.start(t0);
  const stopAt = endTime + tail + 0.08;
  s1.stop(stopAt); s2.stop(stopAt); s3.stop(stopAt);

  scheduleCleanup([s1,s2,s3,g1,g2,g3,mix,env,hp,glass,dark,dry,wet,outTrim, ...room.nodes], durMs + tail*1000 + 1000);
  return { out: outTrim, nodes:[s1,s2,s3,g1,g2,g3,mix,env,hp,glass,dark,dry,wet,outTrim, ...room.nodes] };
}

function reedGardenNote(ctx, freq, durMs, vel=0.84){
  const t0 = ctx.currentTime;
  const dur = Math.max(0.10, durMs/1000);
  const f0 = clamp(freq, 60, 2200);
  const v = clamp(vel, 0.14, 1.0);

  const o1 = ctx.createOscillator(); o1.type = 'square'; o1.frequency.value = f0;
  const o2 = ctx.createOscillator(); o2.type = 'square'; o2.frequency.value = f0 * 2.0;
  const o3 = ctx.createOscillator(); o3.type = 'triangle'; o3.frequency.value = f0 * 1.003;
  const g1 = ctx.createGain(); g1.gain.value = 0.25;
  const g2 = ctx.createGain(); g2.gain.value = 0.11;
  const g3 = ctx.createGain(); g3.gain.value = 0.19;
  const mix = ctx.createGain();
  ...
  env.gain.setValueAtTime(0.0001, t0);
  env.gain.exponentialRampToValueAtTime(peak, t0 + 0.006);
  env.gain.setValueAtTime(peak * 0.82, t0 + 0.08);
  const endTime = t0 + dur;
  env.gain.setValueAtTime(peak * 0.68, endTime);
  const tail = clamp(0.55 + dur * 0.36, 0.70, 2.40);
  env.gain.exponentialRampToValueAtTime(0.0001, endTime + tail);

  const hp = ctx.createBiquadFilter(); hp.type = 'highpass'; hp.frequency.value = 120;
  const body = ctx.createBiquadFilter(); body.type = 'peaking'; body.frequency.value = 760; body.Q.value = 0.95; body.gain.value = 2.5;
  const nasal = ctx.createBiquadFilter(); nasal.type = 'peaking'; nasal.frequency.value = 1450; nasal.Q.value = 1.4; nasal.gain.value = 2.2;
  const lp = ctx.createBiquadFilter(); lp.type = 'lowpass'; lp.frequency.value = 3300; lp.Q.value = 0.8;
  const room = makeSoftRoom(ctx);
  const dry = ctx.createGain(); dry.gain.value = 0.86;
  const wet = ctx.createGain(); wet.gain.value = 0.06;
  const outTrim = ctx.createGain(); outTrim.gain.value = 0.66;

  o1.connect(g1); g1.connect(mix);
  o2.connect(g2); g2.connect(mix);
  o3.connect(g3); g3.connect(mix);
  mix.connect(hp); hp.connect(body); body.connect(nasal); nasal.connect(lp); lp.connect(env);
  env.connect(dry); dry.connect(outTrim);
  env.connect(room.in); room.wet.connect(wet); wet.connect(outTrim);
  ...
  scheduleCleanup([o1,o2,o3,g1,g2,g3,mix,env,hp,body,nasal,lp,dry,wet,outTrim, ...room.nodes], (durMs + tail*1000 + 900));
  return { out: outTrim, nodes:[o1,o2,o3,g1,g2,g3,mix,env,hp,body,nasal,lp,dry,wet,outTrim, ...room.nodes] };
}

function tineBloomNote(ctx, freq, durMs, vel=0.84){
  const t0 = ctx.currentTime;
  const dur = Math.max(0.10, durMs/1000);
  const f0 = clamp(freq, 70, 2400);
  const v = clamp(vel, 0.14, 1.0);

  const car = ctx.createOscillator(); car.type = 'sine'; car.frequency.value = f0;
  const mod = ctx.createOscillator(); mod.type = 'sine'; mod.frequency.value = f0 * 3.6;
  const bell = ctx.createOscillator(); bell.type = 'triangle'; bell.frequency.value = f0 * 2.0;
  const sub = ctx.createOscillator(); sub.type = 'triangle'; sub.frequency.value = Math.max(45, f0 * 0.5);
  const modGain = ctx.createGain();
  modGain.gain.setValueAtTime(f0 * 1.6, t0);
  modGain.gain.exponentialRampToValueAtTime(f0 * 0.10, t0 + 0.12);
  mod.connect(modGain); modGain.connect(car.frequency);

  const carG = ctx.createGain(); carG.gain.value = 0.48;
  const bellG = ctx.createGain(); bellG.gain.value = 0.20;
  ...
  env.gain.setValueAtTime(0.0001, t0);
  env.gain.exponentialRampToValueAtTime(peak, t0 + 0.004);
  env.gain.setValueAtTime(peak * 0.76, t0 + 0.06);
  const endTime = t0 + dur;
  env.gain.setValueAtTime(peak * 0.60, endTime);
  const tail = clamp(0.90 + dur * 0.44, 1.10, 3.20);
  env.gain.exponentialRampToValueAtTime(0.0001, endTime + tail);

  const hp = ctx.createBiquadFilter(); hp.type = 'highpass'; hp.frequency.value = 90;
  const body = ctx.createBiquadFilter(); body.type = 'peaking'; body.frequency.value = 480; body.Q.value = 0.95; body.gain.value = 1.4;
  const tine = ctx.createBiquadFilter(); tine.type = 'peaking'; tine.frequency.value = 2450; tine.Q.value = 1.2; tine.gain.value = 3.1;
  const lp = ctx.createBiquadFilter(); lp.type = 'lowpass'; lp.frequency.value = 5600; lp.Q.value = 0.8;
  const chorus = ctx.createDelay(0.05); chorus.delayTime.value = 0.013;
  const chorusGain = ctx.createGain(); chorusGain.gain.value = 0.16;
  const room = makeSoftRoom(ctx);
  const dry = ctx.createGain(); dry.gain.value = 0.80;
  const wet = ctx.createGain(); wet.gain.value = 0.08;
  const outTrim = ctx.createGain(); outTrim.gain.value = 0.62;

  car.connect(carG); carG.connect(mix);
  bell.connect(bellG); bellG.connect(mix);
  sub.connect(subG); subG.connect(mix);
  mix.connect(hp); hp.connect(body); body.connect(tine); tine.connect(lp); lp.connect(env);
  env.connect(dry); dry.connect(outTrim);
  env.connect(chorus); chorus.connect(chorusGain); chorusGain.connect(outTrim);
  env.connect(room.in); room.wet.connect(wet); wet.connect(outTrim);

  car.start(t0); mod.start(t0); bell.start(t0); sub.start(t0);
  const stopAt = endTime + tail + 0.10;
  car.stop(stopAt); mod.stop(stopAt); bell.stop(stopAt); sub.stop(stopAt);

  scheduleCleanup([car,mod,bell,sub,modGain,carG,bellG,subG,mix,env,hp,body,tine,lp,chorus,chorusGain,dry,wet,outTrim, ...room.nodes], (durMs + tail*1000 + 1100));
  return { out: outTrim, nodes:[car,mod,bell,sub,modGain,carG,bellG,subG,mix,env,hp,body,tine,lp,chorus,chorusGain,dry,wet,outTrim, ...room.nodes] };
}

function velvetBarkEPNote(ctx, freq, durMs, vel=0.84){
  const t0 = ctx.currentTime;
  const dur = Math.max(0.10, durMs/1000);
  const f0 = clamp(freq, 70, 2400);
  const v = clamp(vel, 0.14, 1.0);

  const tine = ctx.createOscillator(); tine.type = 'triangle'; tine.frequency.value = f0;
  const bark = ctx.createOscillator(); bark.type = 'square'; bark.frequency.value = f0 * 2.0;
  const sub = ctx.createOscillator(); sub.type = 'sine'; sub.frequency.value = Math.max(45, f0 * 0.5);
  const g1 = ctx.createGain(); g1.gain.value = 0.34;
  ...
  env.gain.setValueAtTime(peak * 0.78, t0 + 0.07);
  const endTime = t0 + dur;
  env.gain.setValueAtTime(peak * 0.62, endTime);
  const tail = clamp(0.95 + dur * 0.38, 1.10, 3.00);
  env.gain.exponentialRampToValueAtTime(0.0001, endTime + tail);

  const hp = ctx.createBiquadFilter(); hp.type = 'highpass'; hp.frequency.value = 100;
  const body = ctx.createBiquadFilter(); body.type = 'peaking'; body.frequency.value = 420; body.Q.value = 0.95; body.gain.value = 1.6;
  const barkEq = ctx.createBiquadFilter(); barkEq.type = 'peaking'; barkEq.frequency.value = 1750; barkEq.Q.value = 1.1; barkEq.gain.value = 2.1;
  const lp = ctx.createBiquadFilter(); lp.type = 'lowpass'; lp.frequency.value = 4300; lp.Q.value = 0.8;
  const room = makeSoftRoom(ctx);
  const dry = ctx.createGain(); dry.gain.value = 0.84;
  const wet = ctx.createGain(); wet.gain.value = 0.07;
  const outTrim = ctx.createGain(); outTrim.gain.value = 0.70;

  tine.connect(g1); g1.connect(mix);
  bark.connect(g2); g2.connect(mix);
  sub.connect(g3); g3.connect(mix);
  mix.connect(hp); hp.connect(body); body.connect(barkEq); barkEq.connect(lp); lp.connect(env);
  env.connect(dry); dry.connect(outTrim);
  env.connect(room.in); room.wet.connect(wet); wet.connect(outTrim);

  tine.start(t0); bark.start(t0); sub.start(t0);
  const stopAt = endTime + tail + 0.08;
  tine.stop(stopAt); bark.stop(stopAt); sub.stop(stopAt);

  scheduleCleanup([tine,bark,sub,g1,g2,g3,mix,env,hp,body,barkEq,lp,dry,wet,outTrim, ...room.nodes], durMs + tail*1000 + 1000);
  return { out: outTrim, nodes:[tine,bark,sub,g1,g2,g3,mix,env,hp,body,barkEq,lp,dry,wet,outTrim, ...room.nodes] };
}

function suitcaseSparkNote(ctx, freq, durMs, vel=0.84){
  const t0 = ctx.currentTime;
  const dur = Math.max(0.10, durMs/1000);
  const f0 = clamp(freq, 70, 2300);
  const v = clamp(vel, 0.14, 1.0);

  const sine = ctx.createOscillator(); sine.type = 'sine'; sine.frequency.value = f0;
  const tine = ctx.createOscillator(); tine.type = 'triangle'; tine.frequency.value = f0 * 2.0;
  const warm = ctx.createOscillator(); warm.type = 'sine'; warm.frequency.value = f0 * 1.01;
  ...
  env.gain.setValueAtTime(peak * 0.80, t0 + 0.08);
  const endTime = t0 + dur;
  env.gain.setValueAtTime(peak * 0.58, endTime);
  const tail = clamp(1.05 + dur * 0.44, 1.30, 3.40);
  env.gain.exponentialRampToValueAtTime(0.0001, endTime + tail);

  const hp = ctx.createBiquadFilter(); hp.type = 'highpass'; hp.frequency.value = 85;
  const body = ctx.createBiquadFilter(); body.type = 'peaking'; body.frequency.value = 310; body.Q.value = 0.9; body.gain.value = 2.1;
  const sparkle = ctx.createBiquadFilter(); sparkle.type = 'peaking'; sparkle.frequency.value = 2100; sparkle.Q.value = 1.0; sparkle.gain.value = 0.9;
  const lp = ctx.createBiquadFilter(); lp.type = 'lowpass'; lp.frequency.value = 3600; lp.Q.value = 0.8;
  const chorus = ctx.createDelay(0.05); chorus.delayTime.value = 0.012;
  const chorusGain = ctx.createGain(); chorusGain.gain.value = 0.13;
  const room = makeSoftRoom(ctx);
  const dry = ctx.createGain(); dry.gain.value = 0.82;
  const wet = ctx.createGain(); wet.gain.value = 0.07;
  const outTrim = ctx.createGain(); outTrim.gain.value = 0.68;

  sine.connect(g1); g1.connect(mix);
  tine.connect(g2); g2.connect(mix);
  warm.connect(g3); g3.connect(mix);
  mix.connect(hp); hp.connect(body); body.connect(sparkle); sparkle.connect(lp); lp.connect(env);
  env.connect(dry); dry.connect(outTrim);
  env.connect(chorus); chorus.connect(chorusGain); chorusGain.connect(outTrim);
  env.connect(room.in); room.wet.connect(wet); wet.connect(outTrim);

  sine.start(t0); tine.start(t0); warm.start(t0);
  const stopAt = endTime + tail + 0.08;
  sine.stop(stopAt); tine.stop(stopAt); warm.stop(stopAt);

  scheduleCleanup([sine,tine,warm,g1,g2,g3,mix,env,hp,body,sparkle,lp,chorus,chorusGain,dry,wet,outTrim, ...room.nodes], durMs + tail*1000 + 1000);
  return { out: outTrim, nodes:[sine,tine,warm,g1,g2,g3,mix,env,hp,body,sparkle,lp,chorus,chorusGain,dry,wet,outTrim, ...room.nodes] };
}

function sunsetDynoNote(ctx, freq, durMs, vel=0.84){
  const t0 = ctx.currentTime;
  const dur = Math.max(0.10, durMs/1000);
  const f0 = clamp(freq, 70, 2500);
  const v = clamp(vel, 0.14, 1.0);

  const tine = ctx.createOscillator(); tine.type = 'sine'; tine.frequency.value = f0;
  const bell = ctx.createOscillator(); bell.type = 'triangle'; bell.frequency.value = f0 * 2.98;
  const fm = ctx.createOscillator(); fm.type = 'sine'; fm.frequency.value = f0 * 5.0;
  const modGain = ctx.createGain();
  modGain.gain.setValueAtTime(f0 * 0.85, t0);
  modGain.gain.exponentialRampToValueAtTime(f0 * 0.05, t0 + 0.12);
  fm.connect(modGain); modGain.connect(tine.frequency);
  ...
  env.gain.setValueAtTime(0.0001, t0);
  env.gain.linearRampToValueAtTime(peak, t0 + 0.004);
  env.gain.setValueAtTime(peak * 0.74, t0 + 0.05);
  const endTime = t0 + dur;
  env.gain.setValueAtTime(peak * 0.50, endTime);
  const tail = clamp(0.90 + dur * 0.42, 1.10, 3.00);
  env.gain.exponentialRampToValueAtTime(0.0001, endTime + tail);

  const hp = ctx.createBiquadFilter(); hp.type = 'highpass'; hp.frequency.value = 120;
  const body = ctx.createBiquadFilter(); body.type = 'peaking'; body.frequency.value = 520; body.Q.value = 0.9; body.gain.value = 0.6;
  const tineEq = ctx.createBiquadFilter(); tineEq.type = 'peaking'; tineEq.frequency.value = 2850; tineEq.Q.value = 1.1; tineEq.gain.value = 3.4;
  const lp = ctx.createBiquadFilter(); lp.type = 'lowpass'; lp.frequency.value = 6200; lp.Q.value = 0.8;
  const chorus = ctx.createDelay(0.05); chorus.delayTime.value = 0.017;
  const chorusGain = ctx.createGain(); chorusGain.gain.value = 0.18;
  const room = makeSoftRoom(ctx);
  const dry = ctx.createGain(); dry.gain.value = 0.80;
  const wet = ctx.createGain(); wet.gain.value = 0.06;
  ...
  tine.connect(g1); g1.connect(mix);
  bell.connect(g2); g2.connect(mix);
  mix.connect(hp); hp.connect(body); body.connect(tineEq); tineEq.connect(lp); lp.connect(env);
  env.connect(dry); dry.connect(outTrim);
  env.connect(chorus); chorus.connect(chorusGain); chorusGain.connect(outTrim);
  env.connect(room.in); room.wet.connect(wet); wet.connect(outTrim);

  tine.start(t0); bell.start(t0); fm.start(t0);
  const stopAt = endTime + tail + 0.08;
  tine.stop(stopAt); bell.stop(stopAt); fm.stop(stopAt);

  scheduleCleanup([tine,bell,fm,modGain,g1,g2,mix,env,hp,body,tineEq,lp,chorus,chorusGain,dry,wet,outTrim, ...room.nodes], durMs + tail*1000 + 1000);
  return { out: outTrim, nodes:[tine,bell,fm,modGain,g1,g2,mix,env,hp,body,tineEq,lp,chorus,chorusGain,dry,wet,outTrim, ...room.nodes] };
}
/***********************
CHORD PLAYERS
***********************/
...
function playSingleNoteForInstrument(rawChord, durMs){
  ...
  if(state.instrument === "acoustic"){
    ...
    const n = velvetBarkEPNote(ctx, f, durMs, 0.84);
    n.out.connect(getOutNode());
    ...
    const n = glassLanternNote(ctx, f, durMs, 0.82);
    n.out.connect(getOutNode());
    ...
    const n = skylineStageKeysNote(ctx, f, durMs, 0.86);
    n.out.connect(getOutNode());
    scheduleCleanup([n.out], durMs + 6400);
    ...
    const n = prismPopNote(ctx, f, durMs, 0.80);
    n.out.connect(getOutNode());
    ...
    const n = midnightGlassNote(ctx, f, durMs, 0.78);
    n.out.connect(getOutNode());
    scheduleCleanup([n.out], durMs + 7600);
    ...
    n.out.connect(getOutNode());
    scheduleCleanup([n.out], durMs + 7600);
  }else if(state.instrument === "reedvelvet"){
    const n = reedVelvetNote(ctx, f, durMs, 0.88);
    n.out.connect(getOutNode());
    scheduleCleanup([n.out], durMs + 7400);
  }else if(state.instrument === "sunsetdyno"){
    const n = sunsetDynoNote(ctx, f, durMs, 0.84);
    n.out.connect(getOutNode());
    scheduleCleanup([n.out], durMs + 7800);
  }else{
...
function playChordForInstrument(rawChord, durMs){
  ...
  else if(state.instrument === "electric") playCustomVoiceChord(ch, durMs, fracMul, velvetBarkEPNote, { dryGain:0.32, wetGain:0.06, baseVel:0.84, cleanupMs:8600 });
  else if(state.instrument === "upright") playUprightChord(ch, durMs, fracMul);
  else if(state.instrument === "fmbell") playCustomVoiceChord(ch, durMs, fracMul, glassLanternNote, { dryGain:0.26, wetGain:0.14, baseVel:0.80, cleanupMs:7600 });
  else if(state.instrument === "analog") playAnalogChord(ch, durMs, fracMul);
  else if(state.instrument === "oakgrand") playCustomVoiceChord(ch, durMs, fracMul, studioOakGrandNote, { dryGain:0.38, wetGain:0.11, baseVel:0.92, cleanupMs:8200 });
  else if(state.instrument === "skyline") playCustomVoiceChord(ch, durMs, fracMul, skylineStageKeysNote, { dryGain:0.30, wetGain:0.08, baseVel:0.86, cleanupMs:7200 });
  else if(state.instrument === "tapeglow") playCustomVoiceChord(ch, durMs, fracMul, tapeGlowKeysNote, { dryGain:0.31, wetGain:0.08, baseVel:0.88, cleanupMs:7200 });
  else if(state.instrument === "prismkeys") playCustomVoiceChord(ch, durMs, fracMul, prismPopNote, { dryGain:0.28, wetGain:0.06, baseVel:0.80, cleanupMs:5200 });
  else if(state.instrument === "dreamstate") playCustomVoiceChord(ch, durMs, fracMul, dreamstatePadNote, { dryGain:0.30, wetGain:0.14, baseVel:0.82, cleanupMs:9000 });
  else if(state.instrument === "novasweep") playCustomVoiceChord(ch, durMs, fracMul, novaSweepNote, { dryGain:0.30, wetGain:0.10, baseVel:0.84, cleanupMs:7000 });
  else if(state.instrument === "midglass") playCustomVoiceChord(ch, durMs, fracMul, midnightGlassNote, { dryGain:0.23, wetGain:0.15, baseVel:0.76, cleanupMs:7800 });
  else if(state.instrument === "suitcase") playCustomVoiceChord(ch, durMs, fracMul, suitcaseSparkNote, { dryGain:0.33, wetGain:0.07, baseVel:0.84, cleanupMs:8200 });
  else if(state.instrument === "reedvelvet") playCustomVoiceChord(ch, durMs, fracMul, reedVelvetNote, { dryGain:0.35, wetGain:0.05, baseVel:0.88, cleanupMs:7800 });
  else if(state.instrument === "sunsetdyno") playCustomVoiceChord(ch, durMs, fracMul, sunsetDynoNote, { dryGain:0.31, wetGain:0.09, baseVel:0.84, cleanupMs:8400 });
