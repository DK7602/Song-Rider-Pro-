...
function wireAudioProgressControl(){
  ...
  const timeFromClientX = (clientX) => {
    ...
    if(!rect || rect.width <= 0) return Number(a.currentTime) || 0;
  ...
  const paint = (target) => {
    ...
    const pct = duration > 0 ? clamp((target / duration) * 100, 0, 100) : 0;
    state.audioSeekPreviewSec = target;
    if(el.audioProgressFill) el.audioProgressFill.style.width = pct + "%";
    if(el.audioProgressThumb) el.audioProgressThumb.style.left = pct + "%";
    if(el.audioCurrentTime) el.audioCurrentTime.textContent = formatAudioTime(target);
  ...
  const beginAt = (clientX) => {
    if(!audioReady() || committing) return false;
    const a = state.audioSyncAudio;
    ...
    // Stop playback while the user's finger owns the timeline.
    try{ if(!a.paused) a.pause(); }catch{}
    state.audioSyncPaused = true;
    if(state.audioSyncRaf){
      cancelAnimationFrame(state.audioSyncRaf);
  ...
  const moveAt = (clientX) => {
    if(!state.audioSeekDragging || !audioReady()) return;
    lastClientX = Number(clientX);
    paint(timeFromClientX(lastClientX));
  };

  const finishAt = async (clientX) => {
    if(!state.audioSeekDragging || !audioReady() || committing) return;
    committing = true;
  ...
  // Pointer Events path (Chrome/Edge/desktop Android).
  ...
  document.addEventListener("pointermove", (e) => {
    if(!state.audioSeekDragging || activePointerId == null || e.pointerId !== activePointerId) return;
    e.preventDefault();
    moveAt(e.clientX);
  ...
  document.addEventListener("pointercancel", (e) => {
    if(!state.audioSeekDragging || activePointerId == null || e.pointerId !== activePointerId) return;
    e.preventDefault();
    finishAt(lastClientX);
  }, { passive:false, capture:true });

  // Explicit Touch Events path for Android/WebView builds where Pointer Events
  // are present but do not reliably deliver moves from this sticky header.
  bar.addEventListener("touchstart", (e) => {
    if(!audioReady() || state.audioSeekDragging) return;
    const t = e.changedTouches && e.changedTouches[0];
    if(!t) return;
    e.preventDefault();
    e.stopPropagation();
  ...
  document.addEventListener("touchmove", (e) => {
    if(!state.audioSeekDragging || activeTouchId == null) return;
  ...
  document.addEventListener("touchend", (e) => {
    ...
    const t = changed.find(x => x.identifier === activeTouchId);
    if(!t) return;
    e.preventDefault();
    finishAt(t.clientX);
  }, { passive:false, capture:true });
  ...
  // Plain click fallback: a quick tap that doesn't produce a usable drag still seeks.
  bar.addEventListener("click", async (e) => {
    if(!audioReady() || state.audioSeekDragging || committing) return;
    e.preventDefault();
    e.stopPropagation();
  ...
  bar.addEventListener("keydown", async (e) => {
    if(!audioReady() || committing) return;
    const a = state.audioSyncAudio;
    ...
    e.preventDefault();
    const wasPlaying = !a.paused && !state.audioSyncPaused;
    paint(clamp(target, 0, a.duration));
