/* app.js (FULL REPLACE MAIN v47_audio_player_follows_recording) */
(() => {
...
horseLeft: $("horseLeft"),
  ...
  audioNativeHost: $("audioNativeHost"),
...
function parkAudioNativeHost(){
  if(!el.audioNativeHost) return;
  try{
    const homeParent = el.nowPlaying?.parentElement || el.recordingsList?.parentElement;
    if(!homeParent) return;
    if(el.audioNativeHost.parentElement !== homeParent){
      if(el.nowPlaying && el.nowPlaying.parentElement === homeParent){
        el.nowPlaying.insertAdjacentElement("afterend", el.audioNativeHost);
      }else if(el.recordingsList && el.recordingsList.parentElement === homeParent){
        homeParent.insertBefore(el.audioNativeHost, el.recordingsList);
      }else{
        homeParent.appendChild(el.audioNativeHost);
      }
    }
  }catch{}
}

function placeActiveAudioPlayerNearRecording(){
  if(!el.audioNativeHost) return;
  const recId = String(state.audioSyncRecId || "");
  if(!recId){
    parkAudioNativeHost();
    return;
  }
  ...
  try{
    const rows = el.recordingsList?.querySelectorAll('[data-recording-rec-id]') || [];
    let slot = null;
    for(const row of rows){
      if(String(row.dataset.recordingRecId || "") === recId){
        slot = row.querySelector('.recordingAudioSlot');
        break;
      }
    }
    if(slot){
      if(el.audioNativeHost.parentElement !== slot) slot.appendChild(el.audioNativeHost);
    }else{
      parkAudioNativeHost();
    }
  }catch{
...
function updateAudioProgressUI(){
  ...
  // Keep the native progress/seek controls directly above the recording that is playing.
  placeActiveAudioPlayerNearRecording();

  // Native browser media controls are now the single seek UI.
  // They operate directly on the exact HTMLAudioElement that is playing.
  if(el.audioNativeHost){
    el.audioNativeHost.style.display = active ? "block" : "none";
  }
  if(el.audioProgressWrap){
    el.audioProgressWrap.style.display = "none";
  }
  ...
  const current = Math.max(0, Math.min(Number(a.currentTime) || 0, duration || Infinity));
  if(el.audioCurrentTime) el.audioCurrentTime.textContent = formatAudioTime(current);
...
function audioSyncStopInternal(){
  ...
  if(el.audioNativeHost){
    parkAudioNativeHost();
    try{ el.audioNativeHost.replaceChildren(); }catch{ el.audioNativeHost.innerHTML = ""; }
    el.audioNativeHost.style.display = "none";
  }
...
// Full Song View syllable counts + beat-boundary dividers.
// Reuses the exact card-view syllable counter / beat splitter and an invisible
// mirror so positions follow the browser's real wrapping on phones.
function _fullBeatBoundaryCharIndexes(rawLine){
  const raw = String(rawLine || "");
  const lyric = raw.trim();
  if(!lyric || _fullPerf_isHeadingLine(lyric) || _fullPerf_isChordLine(lyric)) return [];

  // Manual slash splits are already the card-view override. The slash itself
  // is the exact between-beat boundary in Full view.
  if(lyric.includes("/")){
    const out = [];
  ...
  const beats = autosplitBeatsFromLyrics(lyric);
  const beatWordCounts = beats.map(b => tokenizeWords(b).length);
  const wantedWordEnds = [];
  let cumulative = 0;
  for(let i=0;i<3;i++){
    cumulative += Number(beatWordCounts[i] || 0);
    if(cumulative > 0) wantedWordEnds.push(cumulative);
  }
  if(!wantedWordEnds.length) return [];

  // Locate each word's end in the original line so punctuation/spacing still
  // measures exactly like the textarea text.
  const wordEnds = [];
  const re = /\S+/g;
  let m;
  while((m = re.exec(raw))){
  ...
  return wantedWordEnds
    .map(wordCount => wordEnds[wordCount - 1])
    .filter((idx, i, arr) => Number.isFinite(idx) && idx > 0 && idx < raw.length && arr.indexOf(idx) === i)
    .slice(0,3);
}
...
function refreshFullSectionSyllableOverlay(ta, overlay){
  ...
  // character-width estimate—decides where lyric text, beat boundaries, and
  // syllable counts actually land.
  ...
  lines.forEach((raw) => {
    ...
    const boundaryIndexes = _fullBeatBoundaryCharIndexes(visibleText);
    const boundaryMarkers = [];
    let cursor = 0;

    boundaryIndexes.forEach((charIndex) => {
      const safeIndex = clamp(Number(charIndex) || 0, cursor, visibleText.length);
      if(safeIndex > cursor) row.appendChild(document.createTextNode(visibleText.slice(cursor, safeIndex)));
      ...
      bm.style.margin = "0";
      row.appendChild(bm);
      boundaryMarkers.push(bm);

      cursor = safeIndex;
    });
    if(cursor < visibleText.length) row.appendChild(document.createTextNode(visibleText.slice(cursor)));
    ...
    markers.push({ raw, marker, boundaryMarkers });
  });

  overlay.appendChild(mirror);
  const overlayRect = overlay.getBoundingClientRect();

  markers.forEach(({raw, marker, boundaryMarkers}) => {
    const lyric = String(raw || "").trim();
    if(!lyric) return;
    if(_fullPerf_isHeadingLine(lyric)) return;
    if(_fullPerf_isChordLine(lyric)) return;

    // Three subtle dividers = the same four beat groupings used on the cards.
    boundaryMarkers.forEach((bm) => {
      const br = bm.getBoundingClientRect();
      const divider = document.createElement("span");
      divider.className = "fullBeatDivider";
      divider.setAttribute("aria-hidden", "true");
      divider.style.left = `${Math.round(br.left - overlayRect.left + 3)}px`;
      divider.style.top = `${Math.round(br.top - overlayRect.top + 2)}px`;
...
async function renderRecordings(){
  ...
  // The live <audio> element may currently sit inside the active recording row.
  // Park it before rebuilding the list so playback is never destroyed by innerHTML.
  parkAudioNativeHost();
  el.recordingsList.innerHTML = "";
  ...
  mine.forEach(rec => {
    ...
    item.dataset.recordingRecId = String(rec.id || "");
    item.style.display = "flex";
    item.style.flexDirection = "column";
    item.style.gap = "6px";
    item.style.minWidth = "0";

    // The active native player is moved into this slot so its progress bar
    // appears immediately above the recording that is actually playing.
    const audioSlot = document.createElement("div");
    audioSlot.className = "recordingAudioSlot";
    audioSlot.style.width = "100%";
    audioSlot.style.minWidth = "0";
...
async function startAudioSyncFromRec(rec){
  ...
  // Use THIS exact playback element as the visible native Android/Chrome player.
  // Native media controls perform seeking internally, avoiding custom touch-event issues.
  audio.controls = true;
  audio.setAttribute("controls", "");
  audio.setAttribute("playsinline", "");
  audio.className = "srpNativeAudioPlayer";
  if(el.audioNativeHost){
    try{ el.audioNativeHost.replaceChildren(audio); }catch{
      el.audioNativeHost.innerHTML = "";
      el.audioNativeHost.appendChild(audio);
    }
    el.audioNativeHost.style.display = "block";
  }
  ...
  audio.ontimeupdate = () => {
    if(state.audioSyncAudio === audio) refreshAudioPlaybackUI();
  };
  audio.onseeking = () => {
    if(state.audioSyncAudio !== audio) return;
    state.lastAudioTick8 = -1;
    state.lastHorseBar = -1;
  };
  audio.onseeked = () => {
    ...
    state.tick8 = Math.max(0, audioTickFromTimeSec(t));
    state.lastAudioTick8 = -1;
    state.lastHorseBar = -1;
    clearTick();
    try{ if(shouldTickRun()) applyTick(); }catch{}
  ...
  audio.onpause = () => {
    if(state.audioSyncOn && !audio.ended){
      state.audioSyncPaused = true;
      if(state.audioSyncRaf){
        cancelAnimationFrame(state.audioSyncRaf);
        state.audioSyncRaf = null;
...
function wireAudioProgressControl(){
  // Intentionally unused now. Android/Chrome's native <audio controls>
  // is the seek surface, attached to the exact playing media element.
  return;
}
