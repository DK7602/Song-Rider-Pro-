...
function drumHatClosed(vel=1){
  ...
  const out = ctx.createGain();
  out.gain.setValueAtTime(0.0001, t0);
  out.gain.exponentialRampToValueAtTime(0.20 * v, t0 + 0.0016);
  out.gain.exponentialRampToValueAtTime(0.0001, t0 + 0.042);
  ...
  // smoother hat shimmer instead of the old high-pitched plink
  const shimmerFreqs = [5200, 6900, 8400];
  const shimmerMix = ctx.createGain();
  shimmerMix.gain.value = 0.018 * v;

  const oscs = [];
  for(const f of shimmerFreqs){
    const o = ctx.createOscillator();
    o.type = "triangle";
    o.frequency.setValueAtTime(f * (1 + (Math.random()*2-1)*0.0015), t0);
    oscs.push(o);
  ...
  nG.gain.exponentialRampToValueAtTime(0.18 * v, t0 + 0.0016);
  ...
  scheduleCleanup([out,ns,hp,bp,lp,nG,shimmerMix, ...oscs], 240);
}
