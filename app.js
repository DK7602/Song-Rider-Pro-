/* Song Rider Pro - app.js (FULL REPLACE, BOOT-SAFE) */
(function(){
  "use strict";
  ...
  try{
    ...
    if (!tabsEl || !editorRoot){
      setBoot("JS ERROR: missing #tabs or #editorRoot in index.html", false);
      return;
    }
    ...
    function emptyCard(){
      return { notesRaw: ["","","","","","","",""], lyric:"" };
    }
    function emptySection(title){
      var cards = [];
      for (var i=0;i<CARDS_PER_SECTION;i++) cards.push(emptyCard());
    ...
    function loadAll(){
      ...
      var recMeta = lsGet("recordingsMeta", []);
    ...
    function saveAll(){
      ...
      lsSet("recordingsMeta", state.recordingsMeta);
    ...
    if (headshotImg){
      headshotImg.addEventListener("error", function(){
        headshotImg.src = "headshot.png?v=7";
      });
    }
    ...
    function setAutoSplitButton(){
      if (!autoSplitBtn) return;
      autoSplitBtn.textContent = "AutoSplit: " + (state.autoSplit ? "ON" : "OFF");
      if (state.autoSplit) autoSplitBtn.classList.add("active");
      else autoSplitBtn.classList.remove("active");
    }
    function setInstrumentButtons(){
      if (instAcoustic) instAcoustic.classList.toggle("active", state.instrument === "acoustic");
      if (instElectric) instElectric.classList.toggle("active", state.instrument === "electric");
      if (instPiano) instPiano.classList.toggle("active", state.instrument === "piano");
    }
    function setDrumButtons(){
      if (drumRock) drumRock.classList.toggle("active", state.drumStyle === "rock");
      if (drumHardRock) drumHardRock.classList.toggle("active", state.drumStyle === "hardrock");
      if (drumPop) drumPop.classList.toggle("active", state.drumStyle === "pop");
      if (drumRap) drumRap.classList.toggle("active", state.drumStyle === "rap");
    }
    ...
    function roughSyllables(word){
      ...
      var out = [];
      var cur = "";
      var lastV = vowels.indexOf(w[0]) !== -1;
      for (var i=0;i<w.length;i++){
        var ch = w[i];
        var isV = vowels.indexOf(ch) !== -1;
        cur += ch;
        if (lastV && !isV){
    ...
    function timingSlots4(text){
      ...
      for (var i=0;i<4;i++){
        var chunk = units.slice(i*per, (i+1)*per);
        slots[i] = chunk.join(" ");
      }
    ...
    /***********************
     * Capo display (simple token transpose)
     * (keeping this minimal for stability)
     ***********************/
    ...
    function estimateKey(){
      // lightweight: just show most common root in displayed notes
      var proj = getProject();
      var counts = {};
    ...
    function render(){
      ...
      var name = "Full";
      for (var i=0;i<PAGES.length;i++){
        if (PAGES[i].id === state.pageId) name = PAGES[i].name;
      }
      h2.textContent = name;
    ...
    function renderCard(sectionId, idx, card){
      ...
      ta.addEventListener("input", function(){
        ...
        // update timing cells
        for (var j=0;j<4;j++){
          timeCells[j].textContent = t2.slots[j] || "";
        }
        touchProject(); saveAll();
      });
    ...
    renameProjectBtn.addEventListener("click", function(){
      ...
      var name = prompt("Rename Project " + letter + ":", cur);
      if (name === null) return;
      var clean = name.trim();
    ...
    /***********************
     * Drums: for now just button toggles (sound comes next)
     * (This keeps boot stable; we’ll re-add audio after layout is 100% correct.)
     ***********************/
    function toggleDrum(style){
      state.drumStyle = (state.drumStyle === style) ? null : style;
      setDrumButtons();
      saveAll();

      // quick visual blink to confirm it works
      if (headshotWrap){
        headshotWrap.classList.add("blink");
        setTimeout(function(){ headshotWrap.classList.remove("blink"); }, 120);
      }
      setStatus(state.drumStyle ? ("Drums: " + style.toUpperCase()) : "Drums stopped.");
    }
    ...
    /***********************
     * Recordings: placeholder UI (we’ll re-add IDB after layout is confirmed)
     ***********************/
    function renderRecordings(){
      ...
      d.style.color = "#666";
      d.textContent = "Recording UI will appear here (next step).";
      recordingsList.appendChild(d);
    }

    recordBtn.addEventListener("click", function(){
      alert("Recording will be re-enabled next step. (We’re fixing load/render first.)");
    });
