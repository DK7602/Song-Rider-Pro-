...
async function ensurePdfJs(){
  // ✅ Robust PDF.js loader (works on GitHub Pages + Android Chrome/PWA)
  // Key change: DO NOT pre-fetch CDN URLs (CORS can block fetch even though <script> would load fine).
  ...
  const setWorkerSrc = (preferred) => {
    try{
      const lib = window.pdfjsLib;
      if(!lib) return;
      lib.GlobalWorkerOptions = lib.GlobalWorkerOptions || {};
      if(!lib.GlobalWorkerOptions.workerSrc && preferred){
        lib.GlobalWorkerOptions.workerSrc = preferred;
      }
    }catch(e){}
  };

  // 1) Try local pdf.min.js (repo root/lib)
  for(const jsUrl of localCandidates){
    try{
      await withTimeout(loadScript(jsUrl), 15000, "Timeout loading: " + jsUrl);
      if(window.pdfjsLib?.getDocument){
        // worker optional because we set disableWorker:true later, but set if present
        setWorkerSrc(asUrl("pdf.worker.min.js?v="+stamp));
        return;
      }
    }catch(e){}
  }

  // 2) Try CDN classic (works even when fetch/CORS would fail)
  for(const cdn of cdnCandidates){
    try{
      await withTimeout(loadScript(cdn.js), 15000, "Timeout loading: " + cdn.js);
      if(window.pdfjsLib?.getDocument){
        setWorkerSrc(cdn.worker);
        return;
      }
    }catch(e){}
  }

  throw new Error("PDF.js failed to load. (Tried local pdf.min.js and CDN fallbacks.)");
}}
await ensurePdfJs();
