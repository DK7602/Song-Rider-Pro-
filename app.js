...
function mergeProjectPreservingNotes(project, existing=null){
  const next = normalizeProject(deepClone(project));
  const prev = existing ? normalizeProject(deepClone(existing)) : null;

  const bestNotes = pickBestNotesBackup([
    {
      notesDoc: String(next?.notesDoc || ""),
      notesColor: String(next?.notesColor || ""),
      notesUpdatedAt: Number(next?.notesUpdatedAt) || 0,
      updatedAt: Number(next?.notesUpdatedAt) || 0
    },
    prev ? {
      notesDoc: String(prev?.notesDoc || ""),
      notesColor: String(prev?.notesColor || ""),
      notesUpdatedAt: Number(prev?.notesUpdatedAt) || 0,
      updatedAt: Number(prev?.notesUpdatedAt) || 0
    } : null,
    readNotesBackup(next?.id)
  ]);

  if(bestNotes){
    next.notesDoc = String(bestNotes.notesDoc || "");
    next.notesUpdatedAt = Number(bestNotes.notesUpdatedAt ?? bestNotes.updatedAt) || next.notesUpdatedAt || 0;
    if(String(bestNotes.notesColor || "").trim()) next.notesColor = String(bestNotes.notesColor || next.notesColor || "#151515");
  }

  const nextImages = Array.isArray(next?.notesImages) ? next.notesImages : [];
  const prevImages = Array.isArray(prev?.notesImages) ? prev.notesImages : [];
  if(!nextImages.length && prevImages.length){
    next.notesImages = deepClone(prevImages);
  }
...
function upsertProject(p){
  ...
  let next = mergeProjectPreservingNotes(p, existing);
  next.updatedAt = now();
  if(i >= 0) all[i] = next;
  else all.unshift(next);
  saveAllProjects(all);
  writeNotesBackup(next);
  if(state.project && String(state.project.id || "") === String(next.id || "")) state.project = normalizeProject(deepClone(next));
  setCurrentProjectIdForScope(getStorageScope(), next.id);
...
const studioOakGrandNote = makeVariantKeyNote(grandPianoNote, {
  hp: 52, bodyFreq: 300, bodyGain: 1.8, presenceFreq: 2700, presenceGain: 1.2, lp: 6100, roomMix: 0.08, outGain: 0.98
});
const skylineKeysNote = makeVariantKeyNote(grandPianoNote, {
  hp: 88, bodyFreq: 420, bodyGain: 0.8, presenceFreq: 3100, presenceGain: 2.8, lp: 7200, chorusMs: 11, chorusMix: 0.16, outGain: 0.88
});
const tapeGlowKeysNote = makeVariantKeyNote(uprightPianoNote, {
  hp: 72, bodyFreq: 260, bodyGain: 2.6, presenceFreq: 1850, presenceGain: 0.7, lp: 3500, roomMix: 0.06, outGain: 0.94
});
const prismKeysNote = makeVariantKeyNote(fmBellKeysNote, {
  hp: 110, bodyFreq: 520, bodyGain: 0.5, presenceFreq: 3600, presenceGain: 2.6, lp: 7600, chorusMs: 9, chorusMix: 0.13, outGain: 0.80
});
const dreamstatePadNote = makeVariantKeyNote(analogPolyKeysNote, {
  hp: 90, bodyFreq: 640, bodyGain: 1.2, presenceFreq: 2400, presenceGain: 0.8, lp: 4300, chorusMs: 15, chorusMix: 0.20, roomMix: 0.10, outGain: 0.82
});
const novaSweepNote = makeVariantKeyNote(synthKeysNote, {
  hp: 95, bodyFreq: 500, bodyGain: 1.0, presenceFreq: 2800, presenceGain: 2.0, lp: 6500, chorusMs: 8, chorusMix: 0.12, outGain: 0.86
});
const midnightGlassNote = makeVariantKeyNote(fmBellKeysNote, {
  hp: 130, bodyFreq: 760, bodyGain: 0.6, presenceFreq: 4100, presenceGain: 3.2, lp: 8200, roomMix: 0.12, outGain: 0.76
});
const suitcaseSparkNote = makeVariantKeyNote(pianoNote, {
  hp: 82, bodyFreq: 360, bodyGain: 1.5, presenceFreq: 2100, presenceGain: 1.2, lp: 4200, chorusMs: 10, chorusMix: 0.14, outGain: 0.88
});
const reedVelvetNote = makeVariantKeyNote(uprightPianoNote, {
  hp: 75, bodyFreq: 300, bodyGain: 2.8, presenceFreq: 1700, presenceGain: 0.8, lp: 3300, outGain: 0.96
});
const sunsetDynoNote = makeVariantKeyNote(pianoNote, {
  hp: 96, bodyFreq: 430, bodyGain: 0.9, presenceFreq: 2900, presenceGain: 1.8, lp: 5200, chorusMs: 12, chorusMix: 0.18, roomMix: 0.07, outGain: 0.84
});
...
function getInstrumentVoiceConfig(which){
  const map = {
    acoustic:   { maker: grandPianoNote,    vel: 0.94, cleanup: 5200 },
    upright:    { maker: uprightPianoNote,  vel: 0.90, cleanup: 5600 },
    electric:   { maker: pianoNote,         vel: 0.84, cleanup: 6400 },
    fmbell:     { maker: fmBellKeysNote,    vel: 0.82, cleanup: 6200 },
    analog:     { maker: analogPolyKeysNote,vel: 0.84, cleanup: 6800 },
    piano:      { maker: synthKeysNote,     vel: 0.86, cleanup: 4200 },
    oakgrand:   { maker: studioOakGrandNote,vel: 0.92, cleanup: 7600 },
    skyline:    { maker: skylineKeysNote,   vel: 0.88, cleanup: 6400 },
    tapeglow:   { maker: tapeGlowKeysNote,  vel: 0.90, cleanup: 7200 },
    prismkeys:  { maker: prismKeysNote,     vel: 0.82, cleanup: 7200 },
    dreamstate: { maker: dreamstatePadNote, vel: 0.82, cleanup: 8600 },
    novasweep:  { maker: novaSweepNote,     vel: 0.84, cleanup: 6600 },
    midglass:   { maker: midnightGlassNote, vel: 0.80, cleanup: 7600 },
    suitcase:   { maker: suitcaseSparkNote, vel: 0.84, cleanup: 7600 },
    reedvelvet: { maker: reedVelvetNote,    vel: 0.88, cleanup: 7400 },
    sunsetdyno: { maker: sunsetDynoNote,    vel: 0.84, cleanup: 7800 }
  };
  return map[String(which || "piano")] || map.piano;
}
...
function playSingleNoteForInstrument(rawChord, durMs){
  ...
  const voice = getInstrumentVoiceConfig(state.instrument);
  const n = voice.maker(ctx, f, durMs, voice.vel);
  n.out.connect(getOutNode());
  scheduleCleanup([n.out], durMs + voice.cleanup);
}
...
function playChordForInstrument(rawChord, durMs){
  ...
  else if(state.instrument === "skyline") playCustomVoiceChord(ch, durMs, fracMul, skylineKeysNote, { dryGain:0.33, wetGain:0.07, baseVel:0.86, cleanupMs:6600 });
  ...
  else if(state.instrument === "prismkeys") playCustomVoiceChord(ch, durMs, fracMul, prismKeysNote, { dryGain:0.30, wetGain:0.12, baseVel:0.82, cleanupMs:7600 });
  ...
  else if(state.instrument === "midglass") playCustomVoiceChord(ch, durMs, fracMul, midnightGlassNote, { dryGain:0.29, wetGain:0.14, baseVel:0.80, cleanupMs:8200 });
  else if(state.instrument === "suitcase") playCustomVoiceChord(ch, durMs, fracMul, suitcaseSparkNote, { dryGain:0.32, wetGain:0.08, baseVel:0.84, cleanupMs:8200 });
  else if(state.instrument === "reedvelvet") playCustomVoiceChord(ch, durMs, fracMul, reedVelvetNote, { dryGain:0.34, wetGain:0.06, baseVel:0.88, cleanupMs:7800 });
  else if(state.instrument === "sunsetdyno") playCustomVoiceChord(ch, durMs, fracMul, sunsetDynoNote, { dryGain:0.31, wetGain:0.10, baseVel:0.84, cleanupMs:8400 });
