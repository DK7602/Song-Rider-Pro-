...
const studioOakGrandNote = makeVariantKeyNote(grandPianoNote, {
  hp: 38, bodyFreq: 235, bodyGain: 2.8, presenceFreq: 2200, presenceGain: 0.7, lp: 5600, roomMix: 0.13, outGain: 1.03
});
const skylineKeysNote = makeVariantKeyNote(grandPianoNote, {
  hp: 125, bodyFreq: 620, bodyGain: 0.3, presenceFreq: 3600, presenceGain: 3.8, lp: 8000, chorusMs: 7, chorusMix: 0.12, outGain: 0.82
});
...
const prismKeysNote = makeVariantKeyNote(fmBellKeysNote, {
  hp: 170, bodyFreq: 1050, bodyGain: -0.1, presenceFreq: 5150, presenceGain: 4.8, lp: 9800, chorusMs: 4, chorusMix: 0.07, outGain: 0.70
});
...
const midnightGlassNote = makeVariantKeyNote(fmBellKeysNote, {
  hp: 220, bodyFreq: 1400, bodyGain: -0.5, presenceFreq: 6100, presenceGain: 5.4, lp: 10500, roomMix: 0.20, outGain: 0.66
});
const suitcaseSparkNote = makeVariantKeyNote(pianoNote, {
  hp: 92, bodyFreq: 320, bodyGain: 2.4, presenceFreq: 1750, presenceGain: 0.6, lp: 3850, chorusMs: 12, chorusMix: 0.19, outGain: 0.92
});
const reedVelvetNote = makeVariantKeyNote(uprightPianoNote, {
  hp: 70, bodyFreq: 300, bodyGain: 2.6, presenceFreq: 1350, presenceGain: -0.2, lp: 2550, outGain: 0.98
});
const sunsetDynoNote = makeVariantKeyNote(pianoNote, {
  hp: 126, bodyFreq: 500, bodyGain: 0.5, presenceFreq: 3200, presenceGain: 2.6, lp: 6100, chorusMs: 17, chorusMix: 0.24, roomMix: 0.05, outGain: 0.84
});
...
function playSingleNoteForInstrument(rawChord, durMs){
  ...
  if(state.instrument === "acoustic"){
    ...
    const n = prismKeysNote(ctx, f, durMs, 0.80);
    n.out.connect(getOutNode());
...
function playChordForInstrument(rawChord, durMs){
  ...
  else if(state.instrument === "skyline") playCustomVoiceChord(ch, durMs, fracMul, skylineKeysNote, { dryGain:0.29, wetGain:0.06, baseVel:0.86, cleanupMs:6600 });
  ...
  else if(state.instrument === "prismkeys") playCustomVoiceChord(ch, durMs, fracMul, prismKeysNote, { dryGain:0.25, wetGain:0.09, baseVel:0.78, cleanupMs:7200 });
  else if(state.instrument === "reedgarden") playCustomVoiceChord(ch, durMs, fracMul, reedGardenNote, { dryGain:0.34, wetGain:0.04, baseVel:0.84, cleanupMs:7000 });
  ...
  else if(state.instrument === "tinebloom") playCustomVoiceChord(ch, durMs, fracMul, tineBloomNote, { dryGain:0.30, wetGain:0.09, baseVel:0.84, cleanupMs:8000 });
