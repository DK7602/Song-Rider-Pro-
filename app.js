...
async function extractTextFromPdfFile(file){
  ...
  async function ensurePdfJs(){
    // Self-healing PDF.js loader for GitHub Pages + Android PWA.
    // 1) Prefer global loaded from index.html
    ...
    const tryLoad = (src)=>new Promise((res, rej)=>{
      const s=document.createElement("script");
      s.src = src + (src.includes("?") ? "&v="+stamp : "?v="+stamp);
      s.async = true;
      s.crossOrigin = "anonymous";
    ...
    // 3) If still missing and a service worker is controlling the page, nuke SW + caches and retry once.
    if(!window.pdfjsLib?.getDocument && navigator.serviceWorker?.controller){
      try{
