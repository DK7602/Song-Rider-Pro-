...
// BUILD: PDF_FINAL_20260305
try{ window.__SRP_BUILD__ = "PDF_FINAL_20260305"; }catch(e){}

try{
  const p = new URLSearchParams(location.search);
  if(p.get("nosw")==="1" && "serviceWorker" in navigator){
...
async function extractTextFromPdfFile(file){
  ...
  async async function ensurePdfJs(){
    // ✅ Robust PDF.js loader (GitHub Pages + Android Chrome/PWA safe)
    // We DO NOT "preflight" with fetch() because CORS/caching can block fetch
    // even when <script src> loads fine on mobile.
    ...
    const loadScript = (src) => withTimeout(new Promise((res, rej) => {
      const s = document.createElement("script");
      s.src = src + (src.includes("?") ? "" : ("?v=" + stamp));
      s.async = true;
      s.crossOrigin = "anonymous";
      s.onload = () => res(true);
      s.onerror = () => rej(new Error("Failed to load script: " + src));
    ...
    // Try local first (repo root / lib), then CDN.
    const local = [
      asUrl("pdf.min.js"),
      asUrl("./pdf.min.js"),
      asUrl("lib/pdf.min.js"),
      asUrl("./lib/pdf.min.js"),
    ];
    ...
    for(const u of local){
      try{
        await loadScript(u);
        if(window.pdfjsLib?.getDocument) break;
      }catch(e){}
    }

    // 2) CDN attempts
    let preferredWorker = null;
    if(!window.pdfjsLib?.getDocument){
      for(const c of cdn){
        try{
          preferredWorker = c.worker;
    ...
    if(!window.pdfjsLib?.getDocument){
      throw new Error("PDF.js failed to load. This usually means the PDF.js script was blocked/cached by the PWA/service-worker. Try opening with ?nosw=1 once, or clear site data, then retry.");
    }

    // Worker is optional because we use disableWorker:true, but set anyway when available.
    try{
      if(window.pdfjsLib.GlobalWorkerOptions){
        window.pdfjsLib.GlobalWorkerOptions.workerSrc =
          preferredWorker || asUrl("./pdf.worker.min.js") || "";
      }
    }catch(e){}
  }
await ensurePdfJs();
...
***********************/
async function init(){
  try{ const h=document.getElementById("sheetHint"); if(h) h.textContent = (h.textContent||"") + "  • vPDF_FIX2"; }catch(e){}
